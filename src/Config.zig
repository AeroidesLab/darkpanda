...
test "Config: explicit Chrome149 profile drives HTTP identity" {
    ...
    try std.testing.expectEqualStrings("149.0.7827.203", config.http_headers.profile.ua_full_version);
    ...
    try std.testing.expectEqualStrings("149.0.7827.203", config.http_headers.profile.full_version_list[0].version);
}
