...
fn configureSocket(socket: posix.socket_t) !void {
    ...
    const idle_opt = switch (builtin.os.tag) {
        .windows, .macos, .ios => posix.TCP.KEEPALIVE,
...
fn handleConnection(self: *Server, socket: posix.socket_t) void {
    ...
    // wreq poll: with it false (tests, pre-handshake), perform skips the
    // poll when there's no in-flight HTTP work — sleeping
...
fn buildJSONVersionResponse(app: *const App, port: u16) ![]const u8 {
    ...
    const profile = app.config.http_headers.profile;
    const product = switch (profile.id) {
        .chrome149 => "Chrome/" ++ lp.ClientProfile.chrome149_full_version,
        .darkpanda => "DarkPanda/1.0",
    };
    const body_format =
        "{{" ++
        "\"Browser\": \"{s}\", " ++
        ...
        "}}";
    const body_args = .{ product, profile.user_agent, host, port };
...
fn expectedJSONBrowserField(app: *const App) []const u8 {
    return switch (app.config.http_headers.profile.id) {
        .chrome149 => "\"Browser\": \"Chrome/" ++ lp.ClientProfile.chrome149_full_version ++ "\"",
        .darkpanda => "\"Browser\": \"DarkPanda/1.0\"",
    };
}

const testing = @import("testing.zig");
test "server: buildJSONVersionResponse" {
    const port = testing.test_app.config.port();
    const res = try buildJSONVersionResponse(testing.test_app, port);
    ...
    try testing.expect(std.mem.indexOf(u8, res, expectedJSONBrowserField(testing.test_app)) != null);
    try testing.expect(std.mem.indexOf(u8, res, "\"Protocol-Version\": \"1.3\"") != null);
    var user_agent_buf: [512]u8 = undefined;
    const expected_user_agent = try std.fmt.bufPrint(
        &user_agent_buf,
        "\"User-Agent\": \"{s}\"",
        .{testing.test_app.config.http_headers.profile.user_agent},
    );
    try testing.expect(std.mem.indexOf(u8, res, expected_user_agent) != null);
    var debugger_url_buf: [128]u8 = undefined;
    const expected_debugger_url = try std.fmt.bufPrint(
        &debugger_url_buf,
        "\"webSocketDebuggerUrl\": \"ws://{s}:{d}/\"",
        .{ testing.test_app.config.advertiseHost(), port },
    );
    try testing.expect(std.mem.indexOf(u8, res, expected_debugger_url) != null);
}
...
test "server: get /json/version" {
    const expected_browser = expectedJSONBrowserField(testing.test_app);
    {
        ...
        try testing.expect(std.mem.indexOf(u8, res1, expected_browser) != null);
        try testing.expect(std.mem.indexOf(u8, res1, "\"Protocol-Version\": \"1.3\"") != null);
        var expected_url_buf: [96]u8 = undefined;
        const expected_url = try std.fmt.bufPrint(
            &expected_url_buf,
            "\"webSocketDebuggerUrl\": \"ws://127.0.0.1:{d}/\"",
            .{testing.testCDPPort()},
        );
        try testing.expect(std.mem.indexOf(u8, res1, expected_url) != null);
    }
...
fn createTestClient() !TestClient {
    const address = std.net.Address.initIp4([_]u8{ 127, 0, 0, 1 }, testing.testCDPPort());
    const stream = try std.net.tcpConnectToAddress(address);

    if (builtin.os.tag == .windows) {
        // Winsock expects a DWORD millisecond timeout, not POSIX timeval.
        const timeout = std.mem.toBytes(@as(u32, 10_000));
...
const TestClient = struct {
    ...
    fn httpRequest(self: *TestClient, req: []const u8) ![]const u8 {
        ...
        while (true) {
            pos += try testSocketRecv(self.stream.handle, self.buf[pos..]);
            if (pos == 0) {
    ...
    fn readWebsocketMessage(self: *TestClient) !?WS.Message {
        while (true) {
            const n = try testSocketRecv(self.stream.handle, self.reader.readBuf());
            if (n == 0) {
                return error.Closed;
            }
            self.reader.len += n;
...
fn testSocketRecv(socket: posix.socket_t, buf: []u8) !usize {
