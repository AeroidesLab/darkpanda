...
pub extern "C" fn html5ever_parse_document(
    ...
    set_quirks_mode_callback: SetQuirksModeCallback,
    ...
    let sink = sink::Sink {
        ...
        set_quirks_mode_callback: Some(set_quirks_mode_callback),
    ...
    parse_opts.tree_builder.iframe_srcdoc = iframe_srcdoc;
...
pub extern "C" fn html5ever_parse_document_with_encoding(
    ...
    let mut parse_opts = ParseOpts::default();
    parse_opts.tree_builder.iframe_srcdoc = iframe_srcdoc;
    parse_document(sink, parse_opts).one(StrTendril::from(decoded.as_ref()));
}
...
pub extern "C" fn encoding_max_utf8_buffer_length(
    ...
    // TextDecoder has an explicit encoding label. Its Web API layer owns BOM
    // removal/ignoreBOM, so the codec must neither sniff nor silently morph to
...
pub extern "C" fn encoding_decoder_new(handle: *const c_void) -> *mut c_void {
    ...
    let encoding: &'static Encoding = unsafe { &*(handle as *const Encoding) };
    let decoder = Box::new(encoding.new_decoder_without_bom_handling());
...
#[no_mangle]
pub extern "C" fn html5ever_get_memory_usage() -> Memory {
    // jemalloc is not available on native Windows. Keep the debug ABI stable;
    // callers still receive an explicit "statistics unavailable" value.
    Memory {
        resident: 0,
        allocated: 0,
    }
...
pub extern "C" fn html5ever_streaming_parser_create(
    ...
    let mut parse_opts = ParseOpts::default();
    parse_opts.tree_builder.iframe_srcdoc = iframe_srcdoc;
    let parser = parse_document(sink, parse_opts);
