...
/// darkpanda_url_free). Returns 0 on success, 1 if `host` is not a valid domain.
#[no_mangle]
pub extern "C" fn darkpanda_url_domain_to_ascii(
...
/// Free a NUL-terminated buffer handed out by darkpanda_url_domain_to_ascii.
#[no_mangle]
pub extern "C" fn darkpanda_url_free(ptr: *mut c_uchar, len: usize) {
    if ptr.is_null() {
        return;
    }
