...
extern "c" fn darkpanda_url_free(ptr: ?[*]u8, len: usize) void;
...
pub fn toAscii(allocator: Allocator, host: []const u8) ![]u8 {
    var out_len: usize = 0;
    var out_ptr: ?[*]u8 = null;
    if (darkpanda_url_domain_to_ascii(host.ptr, host.len, &out_ptr, &out_len) != 0) {
        return error.Idna;
    }
    defer darkpanda_url_free(out_ptr, out_len);
