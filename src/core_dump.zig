...
//! `DARKPANDA_DISABLE_CORE_DUMP` lets an operator drop the cores while leaving the default behavior
...
fn shouldDisable() bool {
    ...
    return std.process.hasEnvVarConstant("DARKPANDA_DISABLE_CORE_DUMP");
}
...
test "core_dump: disabled only when the env var is set" {
    _ = unsetenv(@constCast("DARKPANDA_DISABLE_CORE_DUMP"));
    try testing.expectEqual(false, shouldDisable());

    // Windows has no RLIMIT_CORE equivalent here. Keep the product contract
    // explicit instead of reporting that an unsupported suppression request
    // succeeded. (The Windows setenv shim also treats an empty value as
    // deletion, unlike POSIX.)
    if (builtin.os.tag == .windows) return;

    _ = setenv(@constCast("DARKPANDA_DISABLE_CORE_DUMP"), @constCast(""), 1);
    defer _ = unsetenv(@constCast("DARKPANDA_DISABLE_CORE_DUMP"));
