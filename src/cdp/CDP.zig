...
const IsolatedWorld = struct {
    ...
    pub fn createContext(self: *IsolatedWorld, frame: *Frame) !*js.Context {
        if (self.context == null) {
            ...
            errdefer self.browser.env.destroyContext(ctx);
            // Isolated worlds are separate V8 realms but belong to the same
            // WindowAgent/event loop as their host Document.
            try ctx.setWindowAgentCluster(frame.origin);
            self.context = ctx;
        } else {
...
test "cdp: syncRequest short-circuits after disconnect" {
    ...
    // Request-header release is verified by the transport ownership tests. The latch check returns
