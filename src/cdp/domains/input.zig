...
test "cdp.input: primary commands share Chrome pointer click core and cancellation state" {
    var ctx = try testing.context();
    defer ctx.deinit();
    ...
    _ = try ls.local.compileAndRun(
        \\document.body.innerHTML = '<input id="box" type="checkbox"><button id="cancel">cancel</button>';
        \\const box = document.getElementById('box');
        \\const cancel = document.getElementById('cancel');
        \\window.cdpEvents = [];
        \\window.cdpEventDetails = [];
        ...
        \\    cdpEvents.push(type + ':' + e.isTrusted + ':' + e.constructor.name);
        \\    if (e instanceof MouseEvent) cdpEventDetails.push({type, timeStamp:e.timeStamp, sourceCapabilities:e.sourceCapabilities});
        \\  });
        \\}
        \\window.cancelEvents = [];
        \\for (const type of ['pointerdown','mousedown','focus','focusin','pointerup','mouseup','click']) cancel.addEventListener(type, () => cancelEvents.push(type));
        \\cancel.addEventListener('pointerdown', e => e.preventDefault(), {capture:true});
    , null);

    const box_x = try (try ls.local.compileAndRun("{const r=box.getBoundingClientRect();r.x+r.width/2}", null)).toF64();
    const box_y = try (try ls.local.compileAndRun("{const r=box.getBoundingClientRect();r.y+r.height/2}", null)).toF64();
    try ctx.processMessage(.{ .id = 1, .method = "Input.dispatchMouseEvent", .params = .{ .type = "mouseMoved", .x = box_x, .y = box_y } });
    try ctx.processMessage(.{ .id = 2, .method = "Input.dispatchMouseEvent", .params = .{ .type = "mousePressed", .x = box_x, .y = box_y, .button = "left", .clickCount = 1 } });
    try ctx.processMessage(.{ .id = 3, .method = "Input.dispatchMouseEvent", .params = .{ .type = "mouseReleased", .x = box_x, .y = box_y, .button = "left", .clickCount = 1 } });

    const expected =
        "pointerover:true:PointerEvent,pointerenter:true:PointerEvent," ++
        "mouseover:true:MouseEvent,mouseenter:true:MouseEvent," ++
        "pointermove:true:PointerEvent,mousemove:true:MouseEvent," ++
        "pointerdown:true:PointerEvent,mousedown:true:MouseEvent," ++
        "focus:true:FocusEvent,focusin:true:FocusEvent," ++
        "pointerup:true:PointerEvent,mouseup:true:MouseEvent,click:true:PointerEvent";
    try testing.expect((try ls.local.compileAndRun(
        "cdpEvents.join(',') === '" ++ expected ++ "' && box.checked && document.activeElement === box",
        null,
    )).isTrue());
    try testing.expect((try ls.local.compileAndRun(
        \\(() => {
        \\  const byType = Object.fromEntries(cdpEventDetails.map(event => [event.type, event]));
        \\  const mouseTypes = ['mouseover','mouseenter','mousemove','mousedown','mouseup'];
        \\  const pointerTypes = ['pointerover','pointerenter','pointermove','pointerdown','pointerup'];
        \\  const mouseCapabilities = mouseTypes.map(type => byType[type].sourceCapabilities);
        \\  return cdpEventDetails.every(event => typeof event.timeStamp === 'number' && Number.isFinite(event.timeStamp)) &&
        \\    pointerTypes.every(type => byType[type].sourceCapabilities === null) &&
        \\    mouseCapabilities.every(capabilities => capabilities instanceof InputDeviceCapabilities && !capabilities.firesTouchEvents) &&
        \\    mouseCapabilities.every(capabilities => capabilities === mouseCapabilities[0]) &&
        \\    byType.click.sourceCapabilities === mouseCapabilities[0] &&
        \\    byType.pointerdown.timeStamp === byType.mousedown.timeStamp &&
        \\    byType.pointerup.timeStamp === byType.mouseup.timeStamp &&
        \\    byType.mouseup.timeStamp === byType.click.timeStamp;
        \\})()
    , null)).isTrue());

    const cancel_x = try (try ls.local.compileAndRun("{const r=cancel.getBoundingClientRect();r.x+r.width/2}", null)).toF64();
    const cancel_y = try (try ls.local.compileAndRun("{const r=cancel.getBoundingClientRect();r.y+r.height/2}", null)).toF64();
    try ctx.processMessage(.{ .id = 4, .method = "Input.dispatchMouseEvent", .params = .{ .type = "mouseMoved", .x = cancel_x, .y = cancel_y } });
    try ctx.processMessage(.{ .id = 5, .method = "Input.dispatchMouseEvent", .params = .{ .type = "mousePressed", .x = cancel_x, .y = cancel_y, .button = "left", .clickCount = 1 } });
    try ctx.processMessage(.{ .id = 6, .method = "Input.dispatchMouseEvent", .params = .{ .type = "mouseReleased", .x = cancel_x, .y = cancel_y, .button = "left", .clickCount = 1 } });
    try testing.expect((try ls.local.compileAndRun(
        "cancelEvents.join(',') === 'pointerdown,pointerup,click' && document.activeElement === box",
        null,
    )).isTrue());
}
