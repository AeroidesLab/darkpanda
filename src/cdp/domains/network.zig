...
fn canClearBrowserCache(cmd: *CDP.Command) !void {
    ...
    return cmd.sendResult(.{ .result = true }, .{});
}
...
pub fn httpRequestStart(bc: *CDP.BrowserContext, msg: *const Notification.RequestStart) !void {
    ...
    // (e.g. User-Agent) instead of producing a duplicate wire header.
...
test "cdp.network setExtraHTTPHeaders rejects a Mozilla User-Agent smuggled via a colon in the key" {
    ...
    // req.headers.set uses "User-Agent:Mozilla/5.0 (X: Y)", which parses to
