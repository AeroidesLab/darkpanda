...
test "cdp.runtime: tagged opaque values keep inspector subtype and description" {
    const filter: testing.LogFilter = .init(&.{.js});
    defer filter.deinit();
    ...
    // This crosses both InspectorClient virtual callbacks. Chromium omits a
    // custom subtype unless descriptionForValueSubtype also returns non-null.
    try ctx.expectSentResult(.{ .result = .{
        .type = "object",
        .subtype = "node",
        .description = "",
    } }, .{ .id = 61 });
