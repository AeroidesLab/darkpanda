...
fn setBlocking(self: *Connection, blocking: bool) !void {
    if (builtin.os.tag == .windows) {
        var mode: u32 = if (blocking) 0 else 1;
        const ws2 = std.os.windows.ws2_32;
        if (ws2.ioctlsocket(self.socket, ws2.FIONBIO, &mode) == ws2.SOCKET_ERROR) {
            return error.SetSocketModeFailed;
        }
        return;
    }

    const nonblocking = @as(u32, @bitCast(posix.O{ .NONBLOCK = true }));
    const flags = if (blocking) self.socket_flags & ~nonblocking else self.socket_flags;
    _ = try posix.fcntl(self.socket, posix.F.SETFL, flags);
}
...
pub fn init(
    ...
    const socket_flags = if (builtin.os.tag == .windows)
        0
    else
    ...
    if (builtin.os.tag == .windows) {
        var mode: u32 = 1;
        const ws2 = std.os.windows.ws2_32;
        if (ws2.ioctlsocket(socket, ws2.FIONBIO, &mode) == ws2.SOCKET_ERROR) {
            return error.SetSocketModeFailed;
        }
    } else if (builtin.is_test == false) {
        const nonblocking = @as(u32, @bitCast(posix.O{ .NONBLOCK = true }));
...
pub fn send(self: *Connection, data: []const u8) !void {
    ...
    defer if (changed_to_blocking) {
        ...
        self.setBlocking(false) catch |err| {
