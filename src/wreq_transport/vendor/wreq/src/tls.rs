...
pub struct TlsOptions {
    ...
    /// Trust anchor identifiers to request in the TLS `trust_anchors` extension.
    ///
    /// The bytes use the protocol wire format: a sequence of non-empty,
    /// 8-bit-length-prefixed identifiers. `None` omits the extension, while
    /// `Some(empty)` emits an empty identifier vector to advertise retry-flow
    /// support without requesting a specific trust anchor.
    ///
    /// **Default:** `None`
...
impl TlsOptionsBuilder {
    ...
    /// Sets the requested trust anchor identifiers in protocol wire format.
    #[inline]
