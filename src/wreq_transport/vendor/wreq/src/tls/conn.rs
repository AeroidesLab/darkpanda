...
impl TlsConnectorBuilder {
        ...
        T: Into<Cow<'a, TlsOptions>>,
    {
        ...
        // Use BoringSSL's native encoder for trust_anchors. In particular,
        // Some(empty) must remain distinct from None: the former emits an
        // extension containing an empty uint16-length-prefixed ID list.
        if let Some(ref requested_trust_anchors) = opts.requested_trust_anchors {
            #[allow(unsafe_code)]
            let configured = unsafe {
                btls_sys::SSL_CTX_set1_requested_trust_anchors(
                    connector.as_ptr(),
                    requested_trust_anchors.as_ptr(),
                    requested_trust_anchors.len(),
                )
            };
            if configured != 1 {
                return Err(Error::tls(ErrorStack::get()));
            }
        }
