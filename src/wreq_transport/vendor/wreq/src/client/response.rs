...
use crate::client::layer::decoder::EncodedBodySizeMetadata;

/// A live view of the response bytes consumed before content decoding.
///
/// The tracker remains valid after [`Response::bytes_stream`] takes ownership
/// of the response, allowing transport integrations to read the final encoded
/// size only after the decoded stream has reached EOF.
#[cfg(any(
    feature = "gzip",
    feature = "zstd",
    feature = "brotli",
...
#[derive(Clone, Debug)]
pub struct EncodedBodySizeTracker {
...
impl EncodedBodySizeTracker {
    /// Actual encoded body bytes consumed from the HTTP stack so far.
    pub fn observed(&self) -> u64 {
        self.metadata.observed()
    }

    /// Server-declared Content-Length, when present before decoding.
    pub fn declared(&self) -> Option<u64> {
        self.metadata.declared()
    }

    /// Original Content-Encoding removed by transparent decompression.
    pub fn content_encoding(&self) -> Option<&http::HeaderValue> {
...
impl Response {
    ...
    /// Return the counter installed immediately below transparent response
    /// decompression. This is distinct from `content_length()`, which describes
    /// the decoded Body and is intentionally unknown for compressed responses.
    #[cfg(any(
        feature = "gzip",
        feature = "zstd",
        feature = "brotli",
    ...
    pub fn encoded_body_size_tracker(&self) -> Option<EncodedBodySizeTracker> {
        self.res
            .extensions()
            .get::<Arc<EncodedBodySizeMetadata>>()
            .cloned()
            .map(|metadata| EncodedBodySizeTracker { metadata })
    }
