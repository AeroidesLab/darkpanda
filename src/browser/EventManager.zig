...
pub const OrderedHandler = EventManagerBase.OrderedHandler;
...
pub const DispatchOpts = struct {
    ...
    // See EventManagerBase.DispatchDirectOptions.microtask_checkpoint.
    microtask_checkpoint: bool = true,
};
...
pub fn dispatchOpts(self: *EventManager, target: *EventTarget, event: *Event, comptime opts: DispatchOpts) DispatchError!void {
    ...
    switch (target._type) {
        ...
        else => try self.dispatchDirect(target, event, null, .{
            .context = "dispatch",
            .microtask_checkpoint = opts.microtask_checkpoint,
        }),
    }
}
...
fn dispatchNode(self: *EventManager, target: *Node, event: *Event, comptime opts: DispatchOpts) !void {
    ...
    var path_entries: std.ArrayList(Event.DispatchPathEntry) = .empty;
    const original_related_target = event.getRelatedTarget();
    var clear_targets_after_dispatch = false;

    // Activation/default behavior runs after the propagation checkpoint below.
    // In particular, a click microtask still observes checkbox pre-activation,
    // while cancellation rollback and input/change happen afterwards.
    defer {
        ...
        // Execute default action if not prevented. A defer cannot return, so
        // keep the cancelled branch implicit instead of using an empty arm.
        if (!event._prevent_default) {
            if (event._type_string.eql(comptime .wrap("click"))) {
    ...
    // Keep one scope alive for the complete propagation path. Register this
    // cleanup after the activation/default-action cleanup above so LIFO defer
    // order performs the microtask checkpoint first. This matters for legacy
    // click activation: listeners and the microtasks they queue must observe
    // the pre-activated checkbox/radio state; only after that checkpoint may a
    // cancelled click roll back, or a successful click dispatch input/change.
    var ls: js.Local.Scope = undefined;
    frame.js.localScope(&ls);
    defer {
        if (comptime opts.microtask_checkpoint) {
            if (was_handled) {
    ...
    // A listener microtask runs after dispatch has finished, not while the
    // event still appears to be propagating. Register this last so it executes
    // before the checkpoint and before activation/default behavior.
    defer {
        ...
        if (node_path_len > 0) {
            const top_context = path_entries.items[node_path_len - 1];
            event._target = if (clear_targets_after_dispatch) null else top_context.adjusted_target;
            event.setRelatedTarget(if (clear_targets_after_dispatch) null else top_context.adjusted_related_target);
        } else {
            event._target = null;
            event.setRelatedTarget(null);
        }
        event._dispatch_path = &.{};
        self.updateUntrustedMouseOffset(event, null);
        window._current_event = prev_event;
    }

    event._needs_retargeting = false;
    const target_root = target.getRootNode(.{});
    var node: ?*Node = target;
    while (node) |n| {
        try path_entries.append(frame.call_arena, .{
            .invocation_target = n.asEventTarget(),
            .adjusted_target = target.asEventTarget(),
            .adjusted_related_target = original_related_target,
            .closed_roots = try snapshotClosedRoots(n, frame.call_arena),
        });
    ...
    // Target and relatedTarget are retargeted per tree scope before any JS can
    // mutate that topology.  If both collapse to the same target outside a
    // shadow tree, Blink shrinks the path at that boundary.
    adjustTargetsAndRelatedTarget(&path_entries, target, original_related_target, frame);
    node_path_len = path_entries.items.len;
    if (node_path_len > 0) {
        const top = path_entries.items[node_path_len - 1];
        clear_targets_after_dispatch = rootIsShadowRoot(top.adjusted_target) or
            rootIsShadowRoot(top.adjusted_related_target);
    }
    ...
    if (event._type_string.eql(comptime .wrap("load")) == false) {
        const root_is_document = node_path_len > 0 and switch (path_entries.items[node_path_len - 1].invocation_target._type) {
        ...
        if (root_is_document) {
            const top = path_entries.items[node_path_len - 1];
            try path_entries.append(frame.call_arena, .{
                .invocation_target = frame.window.asEventTarget(),
                .adjusted_target = top.adjusted_target,
                .adjusted_related_target = top.adjusted_related_target,
                .closed_roots = &.{},
            });
        }
    }

    const path = path_entries.items;
    event._dispatch_path = path;
    ...
    while (i > 1) {
        ...
        const current_target = context.invocation_target;
        if (self.base.getListeners(current_target, event._type_string)) |list| {
            try self.dispatchPhase(list, context, event, &was_handled, &ls.local, comptime .init(true, opts));
        }
    }
    ...
    const target_context = path[0];
    const target_et = target_context.invocation_target;
    ...
    blk: {
        ...
        if (self.getInlineHandler(target_et, event)) |inline_handler| {
            ...
            const original_related = event.getRelatedTarget();
            event._target = target_context.adjusted_target;
            event.setRelatedTarget(target_context.adjusted_related_target);
            self.updateUntrustedMouseOffset(event, target_et);
            ...
            event.setRelatedTarget(original_related);
        ...
        if (self.base.getListeners(target_et, event._type_string)) |list| {
            try self.dispatchPhase(list, target_context, event, &was_handled, &ls.local, comptime .init(null, opts));
            if (event._stop_propagation) {
                return;
            }
        }
    ...
    if (event._bubbles) {
        ...
        for (path[1..]) |context| {
            ...
            if (self.getInlineHandler(current_target, event)) |inline_handler| {
                ...
                const original_target = event._target;
                const original_related = event.getRelatedTarget();
                event._target = context.adjusted_target;
                event.setRelatedTarget(context.adjusted_related_target);
                self.updateUntrustedMouseOffset(event, current_target);
            ...
            if (self.base.getListeners(current_target, event._type_string)) |list| {
                try self.dispatchPhase(list, context, event, &was_handled, &ls.local, comptime .init(false, opts));
            }
        }
    }
}
...
fn dispatchPhase(self: *EventManager, list: *std.DoublyLinkedList, context: Event.DispatchPathEntry, event: *Event, was_handled: *bool, local: *const js.Local, comptime opts: DispatchPhaseOpts) !void {
    const frame = self.frame;
    ...
    node_loop: while (node) |n| {
        ...
        // The target pair was adjusted when the event path was snapshotted.
        // Never re-read the live DOM here: an earlier listener may have moved
        // either node across a shadow boundary.
        const original_target = event._target;
        const original_related = event.getRelatedTarget();
        event._target = context.adjusted_target;
        event.setRelatedTarget(context.adjusted_related_target);
...
fn updateUntrustedMouseOffset(self: *EventManager, event: *Event, current_target: ?*EventTarget) void {
    const mouse = event.is(MouseEvent) orelse return;
    mouse.updateUntrustedOffset(event._target, current_target, self.frame);
}
...
fn snapshotClosedRoots(node_: *Node, allocator: Allocator) ![]const *Node {
    const ShadowRoot = @import("webapi/ShadowRoot.zig");
    ...
    while (true) {
        ...
        const shadow = root.is(ShadowRoot) orelse break;
...
fn adjustTargetsAndRelatedTarget(
    path: *std.ArrayList(Event.DispatchPathEntry),
    target: *Node,
    original_related_target: ?*EventTarget,
    frame: *Frame,
) void {
    const original_target = target.asEventTarget();
    for (path.items) |*context| {
        context.adjusted_target = getAdjustedTarget(original_target, context.invocation_target);
    }

    const related_target = original_related_target orelse return;
    const related_node = switch (related_target._type) {
        .node => |node| node,
        else => return,
    };
    if (documentIdentity(target, frame) != documentIdentity(related_node, frame)) return;

    for (path.items) |*context| {
        context.adjusted_related_target = getAdjustedTarget(related_target, context.invocation_target);
    }

    const target_root = target.getRootNode(.{});
    const related_root = related_node.getRootNode(.{});
    for (path.items, 0..) |context, i| {
        if (context.adjusted_target != context.adjusted_related_target) continue;

        const adjusted_target_node = switch ((context.adjusted_target orelse continue)._type) {
            .node => |node| node,
            else => continue,
        };
        const adjusted_related_node = switch ((context.adjusted_related_target orelse continue)._type) {
            .node => |node| node,
            else => continue,
        };

        // Preserve dispatch through the original tree's root.  Once both
        // values become identical in an outer tree scope, continuing would
        // leak a transition that occurred wholly inside a shadow tree.
        if (adjusted_target_node.getRootNode(.{}) == target_root and
            adjusted_related_node.getRootNode(.{}) == related_root)
        {
            continue;
        }
        path.shrinkRetainingCapacity(i);
        break;
    }
}

fn documentIdentity(node: *Node, frame: *Frame) *Node {
    if (node._type == .document) return node;
    return (node.ownerDocument(frame) orelse frame.document).asNode();
}
