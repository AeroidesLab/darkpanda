...
/// DOMException wrappers retain their native value after the creating Web IDL
/// callback returns. Custom name/message slices often originate in that
/// callback's resettable call arena, so copy them into the Context lifetime
/// before Local stores the native value behind a V8 wrapper.
pub fn cloneForJs(self: DOMException, allocator: std.mem.Allocator) !DOMException {
    var cloned = self;
    if (self._custom_name) |name| {
        cloned._custom_name = try allocator.dupe(u8, name);
    }
    if (self._custom_message) |message| {
        cloned._custom_message = try allocator.dupe(u8, message);
    }
    return cloned;
}
...
pub fn toString(self: *const DOMException, exec: *js.Execution) ![]const u8 {
    // Web IDL omits the separator whenever either side is empty.
    const name = self.getName();
    const message = self.getMessage();
    if (name.len == 0) return message;
    if (message.len == 0) return name;
    return std.fmt.bufPrint(exec.buf, "{s}: {s}", .{ name, message }) catch return message;
}
