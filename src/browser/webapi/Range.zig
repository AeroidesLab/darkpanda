...
pub fn createContextualFragment(self: *const Range, value: js.Value, frame: *Frame) !*DocumentFragment {
    var context_node = self._proto._start_container;
    const owner_frame = context_node.ownerFrame(frame);
    const compliant = try TrustedTypes.getCompliantString(
        ...
        .{ .operation = .{ .interface = "Range", .name = "createContextualFragment" } },
        .dom_string,
        &frame.js.execution,
    );
    ...
    const fragment = try DocumentFragment.init(owner_frame);
    ...
    const temp_node = if (context_node.is(Node.Element)) |el|
        try Frame.node_factory.createElementNS(owner_frame, el._namespace, el.getTagNameLower(), null)
    else
        try Frame.node_factory.createElementNS(owner_frame, .html, "div", null);

    try owner_frame.parseContextualFragment(temp_node, html);
    ...
    while (temp_node.firstChild()) |child| {
        if (owner_frame.removeNode(temp_node, child, .{ .will_be_reconnected = true }) == .reentered) return fragment;
        try owner_frame.appendNode(fragment_node, child, .{ .child_already_connected = false });
    }
