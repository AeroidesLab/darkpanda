...
pub fn decode(self: *TextDecoder, input_: ?[]const u8, opts_: ?DecodeOpts) ![]const u8 {
    ...
    if (self._decoder) |decoder| {
        ...
        defer {
            // _decode clears _decoder itself on a fatal error.
            if (self._decoder) |active| html5ever.encoding_decoder_free(active);
            self._decoder = null;
            self._bom_seen = false;
        }
...
fn _decode(self: *TextDecoder, input: []const u8, streaming_decoder: ?*anyopaque, is_last: bool) ![]const u8 {
    ...
    if (!self._bom_seen and decoded.len != 0) {
        if (!self._ignore_bom) decoded = stripBom(decoded);
        self._bom_seen = true;
    }
