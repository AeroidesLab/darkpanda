...
_owner_target: ?OwnerMailbox.Target = null,
_change_subscription: ?u64 = null,
...
pub fn attach(self: *CookieStore, exec: *Execution) !void {
    ...
    self._exec = exec;
    errdefer self._exec = null;

    var target = try exec.ownerMailbox().createTarget(self);
    errdefer target.deinit();
    var sender = try target.sender();
    defer sender.deinit();
    const subscription = try exec.session.cookie_jar.subscribeChanges(&sender, deliverPostedChange);
    errdefer exec.session.cookie_jar.unsubscribeChanges(subscription);

    self._owner_target = target;
...
pub fn detach(self: *CookieStore) void {
    ...
    if (self._owner_target) |*target| target.close();
    if (self._change_subscription) |subscription| {
        exec.session.cookie_jar.unsubscribeChanges(subscription);
    }
    if (self._owner_target) |*target| target.deinit();
    self._owner_target = null;
    self._change_subscription = null;
    self._exec = null;
}

fn deliverPostedChange(raw: *anyopaque, owner_raw: *anyopaque) !void {
    const posted: *Cookie.Jar.PostedChange = @ptrCast(@alignCast(raw));
    const self: *CookieStore = @ptrCast(@alignCast(owner_raw));
    const data = posted.notification();
    try onCookieChanged(self, &data);
}
...
fn backendGone(self: *const CookieStore, exec: *const Execution, comptime operation: []const u8) ?js.Promise {
    if (self._exec != null and !exec.isShuttingDown()) return null;

    const local = exec.js.local.?;
    const resolver = local.createPromiseResolver();
    resolver.reject(
        "CookieStore backend gone",
        DOMException.init(
            "Failed to execute '" ++ operation ++ "' on 'CookieStore': CookieStore backend went away",
            "InvalidStateError",
        ),
    );
...
fn accessDenied(exec: *const Execution, comptime operation: []const u8) ?js.Promise {
    if (exec.origin() != null) return null;

    const local = exec.js.local.?;
    const resolver = local.createPromiseResolver();
    resolver.reject(
        "CookieStore access denied",
        DOMException.init(
            "Failed to execute '" ++ operation ++ "' on 'CookieStore': Access to the CookieStore API is denied in this context.",
            "SecurityError",
        ),
    );
    return resolver.promise();
...
pub fn get(self: *CookieStore, input: GetInput, exec: *const Execution) !js.Promise {
    if (accessDenied(exec, "get")) |promise| return promise;
    if (self.backendGone(exec, "get")) |promise| return promise;
    const local = exec.js.local.?;
...
pub fn getAll(self: *CookieStore, input: ?GetInput, exec: *const Execution) !js.Promise {
    if (accessDenied(exec, "getAll")) |promise| return promise;
    if (self.backendGone(exec, "getAll")) |promise| return promise;
    const local = exec.js.local.?;
...
pub fn set(self: *CookieStore, input: SetInput, value: ?[]const u8, exec: *const Execution) !js.Promise {
    if (accessDenied(exec, "set")) |promise| return promise;
    if (self.backendGone(exec, "set")) |promise| return promise;
    const local = exec.js.local.?;
...
pub fn delete(self: *CookieStore, input: DeleteInput, exec: *const Execution) !js.Promise {
    if (accessDenied(exec, "delete")) |promise| return promise;
    if (self.backendGone(exec, "delete")) |promise| return promise;
    const local = exec.js.local.?;
...
fn matchCookies(
    ...
    const cookies = try session.cookie_jar.snapshot(exec.call_arena);
    for (cookies) |*cookie| {
        // CookieStore exposes only cookies that script would see for the
...
test "WebApi: CookieStore cross-owner change delivery" {
    try testing.htmlRunner("cookie_store_cross_owner.html", .{ .timeout_ms = 8_000 });
}
