...
fn throwRequestNotFinished(exec: *Execution, property: []const u8) anyerror {
    const local = exec.js.local.?;
    const message = try std.fmt.allocPrint(
        exec.call_arena,
        "Failed to read the '{s}' property from 'IDBRequest': The request has not finished.",
        .{property},
    );
...
pub fn setError(self: *IDBRequest, err: anyerror) void {
    // Blink's SendError replaces any staged result (notably a cursor reused by
    // continue()) with undefined before dispatching the error event.
    self.clearOwnedResult();
    self._result = .{ .none = js.Undefined{} };
    self._error = err;
    self._error_message = null;
}

pub fn setTransactionAbortError(self: *IDBRequest) void {
    self.setError(error.AbortError);
    self._error_message = "The transaction was aborted, so the request cannot be fulfilled.";
}

// ConnectionCoordinator::OpenRequest::OnForceClose reports this exact Blink
// error for an in-flight open/delete request. Keep it request-local: ordinary
// AbortError results retain the standard DOMException message.
pub fn setConnectionClosedError(self: *IDBRequest) void {
    self._error = error.AbortError;
    self._error_message = "The connection was closed.";
    // Blink's IDBRequest::SendError always replaces an earlier upgrade result
    // with the JavaScript undefined value before dispatching `error`.
    self.clearOwnedResult();
    self._result = .{ .none = js.Undefined{} };
}
...
pub fn getResult(self: *const IDBRequest, exec: *Execution) !JsResult {
    if (self._ready_state != .done) {
        return throwRequestNotFinished(exec, "result");
    }
...
pub fn getError(self: *const IDBRequest, exec: *Execution) !?DOMException {
    if (self._ready_state != .done) {
        return throwRequestNotFinished(exec, "error");
    }
...
pub const Operation = union(enum) {
    ...
    const StoreWrite = struct { store: *IDBObjectStore, kind: IDBObjectStore.WriteKind, value: []const u8, key: IDBObjectStore.PreparedKey };
...
pub fn submit(self: *IDBRequest, op: Operation, exec: *Execution) !*IDBRequest {
    ...
    // Cursor requests are reused. continue()/advance() moves the same request
    // back to pending, so result/error must throw until its next success.
    self._ready_state = .pending;
