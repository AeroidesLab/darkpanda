...
pub fn get(self: *IDBObjectStore, query: js.Value, exec: *Execution) !*IDBRequest {
    try self.assertOperation("get", false, exec);
    const txn = self._txn;
...
pub fn delete(self: *IDBObjectStore, query: js.Value, exec: *Execution) !*IDBRequest {
    try self.assertOperation("delete", true, exec);
    const txn = self._txn;
    const bounds = try IDBKeyRange.resolveKey(txn._arena, query, exec);
    const request = try txn.newRequest();
...
pub fn clear(self: *IDBObjectStore, exec: *Execution) !*IDBRequest {
    try self.assertOperation("clear", true, exec);
    const txn = self._txn;
    const request = try txn.newRequest();
...
pub fn count(self: *IDBObjectStore, query: ?js.Value, exec: *Execution) !*IDBRequest {
    try self.assertOperation("count", false, exec);
    const txn = self._txn;
...
fn _getAll(self: *IDBObjectStore, query_or_options: ?js.Value, count_: ?u32, mode: GetAllMode, operation: []const u8, exec: *Execution) !*IDBRequest {
    try self.assertOperation(operation, false, exec);
    const txn = self._txn;
...
pub fn getKey(self: *IDBObjectStore, query: js.Value, exec: *Execution) !*IDBRequest {
    try self.assertOperation("getKey", false, exec);
    const txn = self._txn;
    const bounds = try IDBKeyRange.resolveKey(txn._arena, query, exec);
    const request = try txn.newRequest();
...
pub fn openCursor(self: *IDBObjectStore, query: ?js.Value, direction: ?IDBCursor.Direction, exec: *Execution) !*IDBRequest {
    try self.assertOperation("openCursor", false, exec);
...
pub fn openKeyCursor(self: *IDBObjectStore, query: ?js.Value, direction: ?IDBCursor.Direction, exec: *Execution) !*IDBRequest {
    try self.assertOperation("openKeyCursor", false, exec);
    const bounds = try IDBKeyRange.resolveQuery(self._txn._arena, query, exec);
...
fn write(self: *IDBObjectStore, value: js.Value, key_arg: ?js.Value, kind: WriteKind, exec: *Execution) !*IDBRequest {
    const operation = @tagName(kind);
    try self.assertOperation(operation, true, exec);
    const txn = self._txn;

    // IndexedDB snapshots the value synchronously. Besides making later
    // author mutations irrelevant, this is the observable point at which an
    // uncloneable value throws DataCloneError; no request is created in that
    // case. Work with a deserialized clone below so key-path getters are not
    // invoked a second time (matching Blink's IDBValueWrapper::Clone path).
    const prefix = try std.fmt.allocPrint(
        exec.call_arena,
        "Failed to execute '{s}' on 'IDBObjectStore': ",
        .{operation},
    );
    const serialized = blk: {
        txn._serializing = true;
        defer txn._serializing = false;
        break :blk value.serializeWithPrefix(prefix) catch return error.TryCatchRethrow;
    };
    defer serialized.deinit();
    const clone = try js.Value.deserialize(exec.js.local.?, serialized.bytes());
    ...
    const prepared: PreparedKey = blk: {
        if (self._key_path) |kp| {
            ...
            if (try Key.extractKeyPath(exec.js.local.?, clone, kp)) |extracted| {
                break :blk .{ .explicit = .{
            ...
            if (Key.canInjectKey(clone, kp.string) == false) {
                return error.DataError;
            }
    ...
    const request = try txn.newRequest();
    const cloned_bytes = try txn._arena.dupe(u8, serialized.bytes());
...
pub fn runWrite(self: *IDBObjectStore, request: *IDBRequest, kind: WriteKind, cloned_bytes: []const u8, prepared: PreparedKey, exec: *Execution) !void {
    self.writeInner(request, kind, cloned_bytes, prepared, exec) catch |err| {
...
fn writeInner(self: *IDBObjectStore, request: *IDBRequest, kind: WriteKind, cloned_bytes: []const u8, prepared: PreparedKey, exec: *Execution) !void {
    const local = exec.js.local.?;
    const value = try js.Value.deserialize(local, cloned_bytes);
    ...
    // Only generated in-line keys mutate the private clone. All other writes
    // can persist the exact bytes captured by the synchronous serialization.
    var injected_serialized: ?js.Value.Serialized = null;
    defer if (injected_serialized) |bytes| bytes.deinit();
    const stored_bytes = switch (prepared) {
        .generate_in_line => blk: {
            injected_serialized = try value.serialize();
            break :blk injected_serialized.?.bytes();
        },
        .generate_out_of_line, .explicit => cloned_bytes,
    };

    // Record + index rows are atomic: a unique-index violation rolls the record
    // write back too.
    try self._engine.savepoint();
    self.writeRecord(kind, encoded, stored_bytes, value, exec) catch |err| {
...
pub fn index(self: *IDBObjectStore, name: []const u8, exec: *Execution) !*IDBIndex {
    if (self._deleted) {
        return throwIndexDOMException(
            exec,
            "InvalidStateError",
            "The object store has been deleted.",
        );
    }
    if (self._txn._settled or self._txn._committing) {
        return throwIndexDOMException(
            exec,
            "InvalidStateError",
    ...
    const txn = self._txn;
    const info = (try self._engine.indexInfo(txn._arena, self._store_id, name)) orelse {
        return throwIndexDOMException(
            exec,
            "NotFoundError",
...
fn throwIndexDOMException(exec: *Execution, name: []const u8, reason: []const u8) anyerror {
    const local = exec.js.local.?;
    const message = try std.fmt.allocPrint(
        exec.call_arena,
        "Failed to execute 'index' on 'IDBObjectStore': {s}",
        .{reason},
    );
...
fn assertOperation(self: *const IDBObjectStore, operation: []const u8, writable: bool, exec: *Execution) !void {
    if (self._deleted) {
    ...
    self._txn.assertActive() catch |err| switch (err) {
        error.TransactionInactiveError => return throwObjectStoreDOMException(
            exec,
            operation,
            "TransactionInactiveError",
            self._txn.inactiveErrorMessage(),
        ),
        else => return err,
    };
    if (writable and self._txn._mode == .readonly) {
        return throwObjectStoreDOMException(
            exec,
            operation,
            "ReadOnlyError",
...
fn throwObjectStoreDOMException(exec: *Execution, operation: []const u8, name: []const u8, reason: []const u8) anyerror {
    const local = exec.js.local.?;
    const message = try std.fmt.allocPrint(
        exec.call_arena,
        "Failed to execute '{s}' on 'IDBObjectStore': {s}",
        .{ operation, reason },
    );
    const exception = try local.zigValueToJs(
        DOMException.init(message, name),
