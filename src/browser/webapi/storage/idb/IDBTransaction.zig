...
// Blink temporarily makes a transaction inactive while structured-cloning a
// put/add value. A getter reached by the serializer therefore cannot enqueue a
// nested IDB operation (or commit the transaction) re-entrantly.
_serializing: bool = false,
_error: ?anyerror = null,
_error_message: ?[]const u8 = null,
// Non-versionchange transactions keep their IDBDatabase connection alive even
// after close() sets close-pending. The connection unregisters only when this
// count contribution is removed at commit/abort/context teardown.
_database_transaction_active: bool = false,
// Embedded in IDBDatabase._transactions while this ordinary transaction keeps
// its frontend connection alive. Versionchange transactions are coordinated
// by the open request and never enter this list.
_database_node: std.DoublyLinkedList.Node = .{},
...
pub fn init(db: *IDBDatabase, mode: Mode, durability: Durability, exec: *Execution) !*IDBTransaction {
    ...
    // Keep a construction pin across scheduler.add(): a stopped scheduler may
    // invoke the task finalizer synchronously, and an add failure drops the
    // task pin before returning. The database bookkeeping must unwind before
    // the transaction arena can be released in either case.
    self.acquireRef();
    defer self.releaseRef(exec.page);
    db.transactionCreated(self);
    self._database_transaction_active = true;
    errdefer self.finishDatabaseTransaction();
...
pub fn deinit(self: *IDBTransaction, page: *Page) void {
    if (comptime IS_DEBUG) {
        ...
        std.debug.assert(self._database_transaction_active == false);
    }
...
pub fn commit(self: *IDBTransaction, exec: *Execution) !void {
    if (self._settled or self._committing) {
        return throwTransactionMethodState(exec, "commit", "The transaction has finished.");
    }
    if (self._serializing or self._active_turn != self._exec.js.scheduler.generation) {
        return throwTransactionMethodState(exec, "commit", "The transaction is not active.");
    }
...
pub fn abort(self: *IDBTransaction, exec: *Execution) !void {
    if (self._settled or self._committing) {
        return throwTransactionMethodState(exec, "abort", "The transaction has finished.");
    }
    return self.abortWith(exec, null);
}

fn throwTransactionMethodState(exec: *Execution, operation: []const u8, reason: []const u8) anyerror {
    const local = exec.js.local.?;
    const message = try std.fmt.allocPrint(
        exec.call_arena,
        "Failed to execute '{s}' on 'IDBTransaction': {s}",
        .{ operation, reason },
    );
    const exception = try local.zigValueToJs(
        DOMException.init(message, "InvalidStateError"),
        .{},
    );
...
pub fn abortWith(self: *IDBTransaction, exec: *Execution, reason: ?anyerror) error{InvalidStateError}!void {
    return self.abortWithOptions(exec, reason, false);
}
...
fn abortWithOptions(
    ...
    exec: *Execution,
    reason: ?anyerror,
    clear_upgrade_before_abort: bool,
) error{InvalidStateError}!void {
    if (self._settled or self._committing) {
        return error.InvalidStateError;
    }

    self.markAborted(reason);
    ...
    for ([_]*std.ArrayList(*IDBRequest){ &self._queue_a, &self._queue_b }) |queue| {
        for (queue.items, 0..) |request, i| {
            // Versionchange operations execute synchronously to preserve schema
            // ordering, so `_op` is already none while their success event is
            // still pending in this queue. Backend abort must replace that
            // undelivered result with AbortError just like Blink's
            // AbortOutstandingRequests; readyState, not operation state, is the
            // observable pending boundary.
            if (i != request._txn_index or request._ready_state == .done) {
                continue;
            }
            request._op = .none;
            request.setTransactionAbortError();
    ...
    // Backend force-close reaches Blink in two phases: outstanding request
    // errors are delivered while IDBDatabase still exposes its versionchange
    // transaction, then OnAbort clears that database slot immediately before
    // dispatching the transaction's abort event. The open request itself keeps
    // exposing request.transaction until its later error event.
    if (clear_upgrade_before_abort and self._mode == .versionchange) {
        self._db._txn = null;
    }
    self.fire(exec, comptime .wrap("abort"), self._on_abort);
}

// Context teardown cannot synchronously dispatch more events into the retired
// realm. It still has to make the transaction inert and close the sqlite
// bracket; the running gate owner releases/hands off only after its current
// native callback unwinds.
pub fn cancelForContextDetach(self: *IDBTransaction) void {
    if (self._settled) return;
    self.markAborted(error.AbortError);
}

// Browser-process backing-store teardown. Unlike context detach, the realm is
// still active, so pending request errors and the transaction's trusted abort
// event remain observable before IDBDatabase dispatches its trusted close.
pub fn forceAbortFromBackend(self: *IDBTransaction, reason: []const u8) void {
    if (self._settled) return;
    // commit() only requests that the queued drain commit as soon as possible;
    // a browser-process force-close still wins over that frontend request.
    // Clear the public abort() guard before entering the shared abort path.
    self._committing = false;
    self._error_message = std.fmt.allocPrint(
        self._arena,
        "Connection is closing because of: {s}",
        .{reason},
    ) catch "Connection is closing because of a backend force-close.";
    self.abortWithOptions(self._exec, error.BackendForceClose, true) catch {};
}

fn markAborted(self: *IDBTransaction, reason: ?anyerror) void {
    self._aborted = true;
    self._settled = true;
    self._error = reason;
    ...
    self.finishDatabaseTransaction();
}

fn finishDatabaseTransaction(self: *IDBTransaction) void {
    if (!self._database_transaction_active) return;
    self._database_transaction_active = false;
    self._db.transactionFinished(self);
}
...
pub fn inactiveErrorMessage(self: *const IDBTransaction) []const u8 {
    return if (self._settled or self._committing)
...
pub fn objectStore(self: *IDBTransaction, name: []const u8, exec: *Execution) !*IDBObjectStore {
    // Blink checks finishing/finished before consulting either its cached
    // object-store map or the transaction scope. A retained store wrapper must
    // therefore not make objectStore() succeed after complete/abort begins.
    if (self._settled or self._committing) {
        return throwObjectStoreDOMException(
            exec,
            "InvalidStateError",
            "The transaction has finished.",
        );
    ...
    if (self._mode != .versionchange) {
        var in_scope = false;
        for (self._scope) |scope_name| {
            if (std.mem.eql(u8, scope_name, name)) {
                in_scope = true;
                break;
            }
        }
        if (!in_scope) return throwObjectStoreNotFound(exec);
    }
    ...
    const info = (try self._engine.objectStoreInfo(self._arena, database_id, name)) orelse {
        return throwObjectStoreNotFound(exec);
    };
...
fn throwObjectStoreNotFound(exec: *Execution) anyerror {
    return throwObjectStoreDOMException(
        exec,
        "NotFoundError",
...
fn throwObjectStoreDOMException(exec: *Execution, name: []const u8, reason: []const u8) anyerror {
    const local = exec.js.local.?;
    const message = try std.fmt.allocPrint(
        exec.call_arena,
        "Failed to execute 'objectStore' on 'IDBTransaction': {s}",
        .{reason},
    );
    const exception = try local.zigValueToJs(
...
pub fn getError(self: *const IDBTransaction) ?DOMException {
    ...
    const exception = DOMException.fromError(mapped) orelse DOMException.init(null, "UnknownError");
    if (self._error_message) |message| {
        return DOMException.init(message, exception.getName());
    }
    return exception;
}
...
fn fire(self: *IDBTransaction, exec: *Execution, typ: lp.String, handler: ?js.Function.Global) void {
    self._settled = true;
    // IndexedDB transaction abort is a trusted bubbling, non-cancelable event;
    // complete stays at-target only. This matters during backend force-close,
    // where the abort can be observed by the owning IDBDatabase before close.
    const options: ?Event.Options = if (typ.eql(comptime .wrap("abort")))
        .{ .bubbles = true }
    else
        null;
    const event = Event.initTrusted(typ, options, exec.page) catch |err| {
...
fn drainInner(self: *IDBTransaction) ?u32 {
    ...
    if (self._queue.items.len > 0) {
        ...
        if (self._settled) {
            // abort() normally released the gate itself. Context detach keeps
            // a running owner in place until the author callback unwinds, so
            // this release is also the deferred handoff for that path.
            _ = self._engine.releaseGate(&self._gate_waiter);
            return null;
        }
        if (self._queue.items.len > 0) {
...
pub const JsApi = struct {
    ...
    // Chrome 149 generated binding declaration order.
    ...
    pub const @"error" = bridge.accessor(IDBTransaction.getError, null, .{});
