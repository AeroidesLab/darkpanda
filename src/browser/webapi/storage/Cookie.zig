...
pub const Jar = struct {
    ...
    change_mutex: std.Thread.Mutex = .{},
    change_subscribers: std.ArrayList(ChangeSubscriber) = .empty,
    next_change_subscriber_id: u64 = 1,

    pub fn init(allocator: Allocator, _: ?*Notification) Jar {
        return .{
            .cookies = .{},
            .allocator = allocator,
        };
    ...
    pub fn deinit(self: *Jar) void {
        ...
        self.change_mutex.lock();
        var subscribers = self.change_subscribers;
        self.change_subscribers = .empty;
        self.change_mutex.unlock();
        for (subscribers.items) |*subscriber| subscriber.sender.deinit();
        subscribers.deinit(std.heap.page_allocator);
    }

    pub const ChangeInvoke = *const fn (data: *anyopaque, owner_context: *anyopaque) anyerror!void;

    const ChangeSubscriber = struct {
        id: u64,
        sender: OwnerMailbox.Sender,
        invoke: ChangeInvoke,
    };

    /// Retain a generation-bound owner sender. Registration may happen from a
    /// Window or Worker owner thread; the page allocator plus this mutex makes
    /// the shared session Jar independent of either realm's allocator.
    pub fn subscribeChanges(self: *Jar, sender: *const OwnerMailbox.Sender, invoke: ChangeInvoke) !u64 {
        var retained = try sender.retain();
        errdefer retained.deinit();

        self.change_mutex.lock();
        defer self.change_mutex.unlock();
        const id = self.next_change_subscriber_id;
        self.next_change_subscriber_id +%= 1;
        try self.change_subscribers.append(std.heap.page_allocator, .{
            .id = id,
            .sender = retained,
            .invoke = invoke,
        });
    ...
    pub fn unsubscribeChanges(self: *Jar, id: u64) void {
        var removed: ?ChangeSubscriber = null;
        self.change_mutex.lock();
        for (self.change_subscribers.items, 0..) |subscriber, i| {
            if (subscriber.id != id) continue;
            removed = self.change_subscribers.swapRemove(i);
            break;
        }
        self.change_mutex.unlock();
        if (removed) |*subscriber| subscriber.sender.deinit();
    }
    ...
    pub fn add(
        ...
        const change: ?ChangePayload = locked: {
            self.mutex.lock();
            defer self.mutex.unlock();
            ...
            for (self.cookies.items, 0..) |*c, i| {
                if (!areCookiesEqual(&cookie, c)) continue;

                // RFC 6265bis 5.7.2: a non-HTTP API (e.g. document.cookie)
                // must not replace an HttpOnly cookie.
                if (c.http_only and !is_http) break :locked null;

                if (is_expired) {
                    // Snapshot every borrowed field before releasing the
                    // cookie's arena. Notification callbacks run only after
                    // the Jar mutex is released.
                    const payload = try ChangePayload.init(.deleted, c);
                    c.deinit();
                    _ = self.cookies.swapRemove(i);
                    break :locked payload;
                }

                c.deinit();
                self.cookies.items[i] = cookie;
                stored = true;
                break :locked try ChangePayload.init(.changed, &self.cookies.items[i]);
            }

            if (is_expired) break :locked null;
            try self.cookies.append(self.allocator, cookie);
            stored = true;
            break :locked try ChangePayload.init(.changed, &self.cookies.items[self.cookies.items.len - 1]);
        };

        if (change) |payload_value| {
            var payload = payload_value;
            defer payload.deinit();
            self.dispatchChange(&payload);
        }
    }
    ...
    const ChangePayload = struct {
        ...
        fn init(kind: Notification.CookieChanged.Kind, cookie: *const Cookie) !ChangePayload {
        ...
        fn deinit(self: *ChangePayload) void {
            const allocator = std.heap.page_allocator;
    ...
    fn dispatchChange(self: *Jar, payload: *const ChangePayload) void {
        var subscribers: std.ArrayList(ChangeSubscriber) = .empty;
        self.change_mutex.lock();
        subscribers.ensureTotalCapacity(
            std.heap.page_allocator,
            self.change_subscribers.items.len,
        ) catch {
            self.change_mutex.unlock();
            return;
        };
        for (self.change_subscribers.items) |subscriber| {
            const sender = subscriber.sender.retain() catch continue;
            subscribers.appendAssumeCapacity(.{
                .id = subscriber.id,
                .sender = sender,
                .invoke = subscriber.invoke,
            });
        }
        self.change_mutex.unlock();
        defer {
        ...
        for (subscribers.items) |*subscriber| {
            const posted = PostedChange.init(payload) catch continue;
            // A same-owner mutation (CookieStore/document.cookie) must hand
            // the notification to CookieStore now so it can enqueue its
            // asynchronous `change` task before later timers. Producers on a
            // different owner (Window <-> Worker or network threads) retain
            // the mailbox's thread-safe FIFO delivery path.
            _ = subscriber.sender.deliverOwned(posted.owned(subscriber.invoke)) catch {};
        }
    }

    pub const PostedChange = struct {
        kind: Notification.CookieChanged.Kind,
        name: []u8,
        value: []u8,
        domain: []u8,
        ...
        fn init(payload: *const ChangePayload) !*PostedChange {
            const allocator = std.heap.page_allocator;
            const self = try allocator.create(PostedChange);
            errdefer allocator.destroy(self);
            self.name = try allocator.dupe(u8, payload.name);
            errdefer allocator.free(self.name);
            self.value = try allocator.dupe(u8, payload.value);
            errdefer allocator.free(self.value);
            self.domain = try allocator.dupe(u8, payload.domain);
            errdefer allocator.free(self.domain);
            self.path = try allocator.dupe(u8, payload.path);
            errdefer allocator.free(self.path);
            self.kind = payload.kind;
            self.secure = payload.secure;
            self.http_only = payload.http_only;
            self.same_site = payload.same_site;
            return self;
        }

        fn owned(self: *PostedChange, invoke: ChangeInvoke) OwnerMailbox.OwnedPayload {
            return .{
                .data = self,
                .invoke = invoke,
                .destroy = destroy,
        ...
        fn destroy(raw: *anyopaque) void {
            const self: *PostedChange = @ptrCast(@alignCast(raw));
            const allocator = std.heap.page_allocator;
            allocator.free(self.name);
            allocator.free(self.value);
            allocator.free(self.domain);
        ...
        pub fn notification(self: *const PostedChange) Notification.CookieChanged {
            return .{
                .kind = self.kind,
                .name = self.name,
                .value = self.value,
    ...
    pub fn removeExpired(self: *Jar, request_time: ?i64) void {
        self.mutex.lock();
        defer self.mutex.unlock();
        self.removeExpiredLocked(request_time);
    }

    fn removeExpiredLocked(self: *Jar, request_time: ?i64) void {
    ...
    pub const LookupOpts = struct {
        ...
        // Migrated network callers can provide the result of the full
        // SiteForCookies + request-initiator calculation. Null preserves the
        // legacy origin_url-only behaviour for existing callers.
        same_site_override: ?bool = null,
    };
    pub fn forRequest(self: *Jar, target_url: [:0]const u8, writer: anytype, opts: LookupOpts) !void {
        ...
        const same_site = opts.same_site_override orelse try areSameSite(opts.origin_url, target.host);

        self.removeExpiredLocked(opts.request_time);
    ...
    /// Deep, caller-owned view of the Jar. No slice escapes the mutex while
    /// still borrowing a Cookie arena that another agent can replace/free.
    pub fn snapshot(self: *Jar, allocator: Allocator) ![]Snapshot {
        self.mutex.lock();
        defer self.mutex.unlock();

        const result = try allocator.alloc(Snapshot, self.cookies.items.len);
        var initialized: usize = 0;
        errdefer {
            for (result[0..initialized]) |*copy| copy.deinit(allocator);
            allocator.free(result);
        }
        for (self.cookies.items, result) |cookie, *copy| {
            copy.* = try Snapshot.init(allocator, &cookie);
            initialized += 1;
        }
        return result;
    }
    ...
    pub const Snapshot = struct {
        ...
        fn init(allocator: Allocator, source: *const Cookie) !Snapshot {
        ...
        fn cookie(self: *const Snapshot) Cookie {
            return .{
                .arena = undefined,
                .name = self.name,
                .value = self.value,
        ...
        pub fn matchesHost(self: *const Snapshot, host: []const u8) bool {
            const value = self.cookie();
            return value.matchesHost(host);
        }

        pub fn appliesTo(self: *const Snapshot, target: *const PreparedUri, same_site: bool, is_navigation: bool, is_http: bool) bool {
            const value = self.cookie();
            return value.appliesTo(target, same_site, is_navigation, is_http);
        }
    };
