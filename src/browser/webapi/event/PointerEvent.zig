...
const TrustedPointerEventOptions = struct {
    pointerId: i32 = 0,
    pointerType: []const u8 = "",
    width: f64 = 1.0,
    height: f64 = 1.0,
...
const TrustedOptions = Event.inheritOptions(
    PointerEvent,
    TrustedPointerEventOptions,
);
...
pub fn initTrusted(typ: []const u8, _opts: ?TrustedOptions, frame: *Frame) !*PointerEvent {
    return initWithTrusted(typ, _opts, true, frame);
}

fn initWithTrusted(typ: []const u8, _opts: anytype, trusted: bool, frame: *Frame) !*PointerEvent {
    ...
    const offset_x = if (comptime @hasField(OptionsType, "offsetX")) opts.offsetX else opts.clientX;
    const offset_y = if (comptime @hasField(OptionsType, "offsetY")) opts.offsetY else opts.clientY;
    const event = try frame._factory.mouseEvent(
        ...
        MouseEvent{
            ...
            ._button = MouseEvent.coerceButton(opts.button),
            ._buttons = opts.buttons,
            ._related_target = opts.relatedTarget,
            ._offset_x = offset_x,
            ._offset_y = offset_y,
            ._movement_x = if (trusted) opts.movementX else MouseEvent.webIdlLong(opts.movementX),
            ._movement_y = if (trusted) opts.movementY else MouseEvent.webIdlLong(opts.movementY),
    ...
    if (trusted) {
        // Pointer events created from a platform WebInputEvent carry a view,
        // even though their sourceCapabilities remains null. The compatibility
        // click path fills sourceCapabilities separately.
        event._proto._proto._view = frame.window;
    }
    return event;
}
