...
pub fn getDisabled(self: *const Style) bool {
    const sheet = self._sheet orelse return false;
    return sheet.getDisabled();
}

pub fn setDisabled(self: *Style, disabled: bool, frame: *Frame) !void {
    const sheet = try self.getSheet(frame) orelse return;
    sheet.setDisabled(disabled, frame);
}
...
pub fn getSheet(self: *Style, frame: *Frame) !?*CSSStyleSheet {
    ...
    if (!self.asNode().isConnected()) {
        if (self._sheet) |sheet| frame.unregisterStyleSheet(sheet);
        self._sheet = null;
        return null;
    }
    ...
    if (self._sheet) |sheet| {
        try frame.registerStyleSheet(sheet);
        return sheet;
    }
...
pub const Build = struct {
    pub fn attributeChange(element: *Element, name: @import("darkpanda").String, _: @import("darkpanda").String, frame: *Frame) !void {
        if (!name.eql(comptime .wrap("type"))) return;
        _ = try element.as(Style).getSheet(frame);
        frame._style_manager.sheetModified();
    }

    pub fn attributeRemove(element: *Element, name: @import("darkpanda").String, frame: *Frame) !void {
        if (!name.eql(comptime .wrap("type"))) return;
        _ = try element.as(Style).getSheet(frame);
        frame._style_manager.sheetModified();
    }
