...
pub fn click(self: *HtmlElement, frame: *Frame) !void {
    ...
    // Chromium exposes the activation event as an untrusted PointerEvent.
    // Browser-owned input uses pointerId=1; HTMLElement.click() uses -1 and
    // an empty pointerType to distinguish the script activation path.
...
// so usage is explicit rather than silently depending on an
...
fn mergeTextNodes(left_node: *Node, right_node: *Node, frame: *Frame) !bool {
    ...
    if (right_node.parentNode()) |p| {
        // remove right node
        _ = frame.removeNode(p, right_node, .{ .will_be_reconnected = false });
    }
    return true;
}
...
fn renderedTextFragment(value: []const u8, frame: *Frame) ![]Node.NodeOrText {
    ...
    while (true) {
        ...
        if (text_end > 0) {
            try nodes.append(arena, .{ .text = .{ .value = rest[0..text_end] } });
        }
        rest = rest[text_end..];
        if (rest.len == 0) {
            return nodes.items;
