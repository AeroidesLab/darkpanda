...
/// Stable browsing-context owner. A same-origin caller may invoke a child
/// Navigation object from another realm, so the bridge-injected Frame alone is
/// not the navigation target.
_owner: ?*Frame = null,
...
fn owner(self: *Navigation, caller: *Frame) *Frame {
...
pub fn pushEntry(
    ...
    entry.* = NavigationHistoryEntry{
        ...
        ._url = url,
        ._state = state,
        ._context = frame._navigation_context,
    };
...
pub fn navigate(self: *Navigation, _url: [:0]const u8, _opts: ?NavigateOptions, caller: *Frame) !NavigationReturn {
    const frame = self.owner(caller);
    const arena = frame._session.arena;
...
pub fn reload(self: *Navigation, _opts: ?ReloadOptions, caller: *Frame) !NavigationReturn {
    const frame = self.owner(caller);
    const arena = frame._session.arena;
...
pub fn traverseTo(self: *Navigation, key: []const u8, _opts: ?TraverseToOptions, caller: *Frame) !NavigationReturn {
    const frame = self.owner(caller);
    if (_opts != null) {
...
pub fn updateCurrentEntry(self: *Navigation, options: UpdateCurrentEntryOptions, caller: *Frame) !void {
    const frame = self.owner(caller);
    const arena = frame._session.arena;
...
pub fn dispatch(self: *Navigation, func: js.Function.Global, event: *Event, frame: *Frame) !void {
    const target = self.owner(frame);
    return target._event_manager.dispatchDirect(
        self.asEventTarget(),
        event,
        func,
        .{ .context = "Navigation" },
...
test "Navigation: about:blank commits entry" {
    ...
    // Exercise a fresh browsing-context history, independent of other pages.
    frame.navigation()._entries.clearRetainingCapacity();
    frame.navigation()._index = 0;
