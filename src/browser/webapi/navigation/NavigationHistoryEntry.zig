...
_context: *Frame.NavigationContext,

pub fn id(self: *const NavigationHistoryEntry) []const u8 {
    return self._id;
}

pub fn index(self: *const NavigationHistoryEntry) i32 {
    const navigation = &self._context.navigation;
...
pub fn sameDocument(self: *const NavigationHistoryEntry, frame: *Frame) bool {
    ...
    const owner = self._context.navigation._owner orelse frame;
    return URL.eqlDocument(got_url, owner.base());
}
