...
// Incremented every time this node is detached or inserted. DOM algorithms
// which cross an author-script boundary use it to distinguish "still under the
// same parent" from remove+reinsert round trips to that same parent.
_tree_mutation_serial: u64 = 0,
...
/// Host-including containment used by DOM's pre-insert and atomic-move cycle
/// checks. A ShadowRoot is not a normal child of its host, so Node.contains()
/// alone would allow inserting a host into its own shadow tree.
pub fn shadowIncludingContains(self: *const Node, child_: ?*const Node) bool {
    var child = child_ orelse return false;
    while (true) {
        ...
        if (@constCast(child).is(ShadowRoot)) |shadow| {
            child = shadow._host.asConstNode();
            continue;
        }
        return false;
    }
...
fn validateNodeInsertion(parent: *Node, node: *Node) !void {
    ...
    // Check host-including ancestry as required by DOM pre-insert. Ordinary
    // containment does not cross ShadowRoot -> host.
    if (node.shadowIncludingContains(parent)) {
        return error.HierarchyError;
    }
...
pub fn appendChild(self: *Node, child: *Node, frame: *Frame) !*Node {
    ...
    var load_guard = frame.beginReconnectLoadGuard(child._parent != null and child_connected and will_be_reconnected);
    ...
    if (child._parent) |parent| {
        ...
        if (frame.removeNode(parent, child, .{
            .will_be_reconnected = will_be_reconnected,
        }) == .reentered) return child;
    }
...
pub fn setTextContent(self: *Node, data: []const u8, frame: *Frame) !void {
    switch (self._type) {
        .element => |el| {
            ...
            return el.replaceChildren(&.{.{ .text = .{ .value = data } }}, frame);
        },
        ...
        .document_fragment => |frag| {
            ...
            return frag.replaceChildren(&.{.{ .text = .{ .value = data } }}, frame);
        },
...
pub fn removeChild(self: *Node, child: *Node, frame: *Frame) !*Node {
    ...
    while (it.next()) |n| {
        if (n == child) {
            frame.domChanged();
            _ = frame.removeNode(self, child, .{ .will_be_reconnected = false });
            return child;
        }
    }
...
pub fn insertBefore(self: *Node, new_node: *Node, ref_node_: ?*Node, frame: *Frame) !*Node {
    ...
    // DOM's pre-insert algorithm replaces a self-reference with the node's
    // next sibling before removal. This is not a no-op: an iframe browsing
    // context is discarded/recreated and observers see remove then add.
    const insertion_ref = if (new_node == ref_node) new_node.nextSibling() else ref_node;
    ...
    frame.domChanged();
    const will_be_reconnected = self.isConnected() and !adopting_to_new_document;
    var load_guard = frame.beginReconnectLoadGuard(new_node._parent != null and child_already_connected and will_be_reconnected);
    defer load_guard.deinit();
    if (new_node._parent) |parent| {
        if (frame.removeNode(parent, new_node, .{ .will_be_reconnected = will_be_reconnected }) == .reentered) return new_node;
    }
...
pub fn replaceChild(self: *Node, new_child: *Node, old_child: *Node, frame: *Frame) !*Node {
    ...
    if (new_child != old_child) {
        _ = frame.removeNode(self, old_child, .{ .will_be_reconnected = false });
    }
...
pub fn moveBefore(self: *Node, node_val: js.Value, child_val: js.Value, frame: *Frame) !void {
    ...
    if (node.shadowIncludingContains(self)) {
        return error.HierarchyError;
    }
    ...
    // `self` and `node` share a composed root, so connectedness does not
    // change. They can still belong to different TreeScopes (Document versus
    // a ShadowRoot), however, so retain the exact old ID maps for migration.
    // Browsing contexts and custom-element connection state remain preserved;
    // MutationObservers and ranges still observe the move.
    const connected = node.isConnected();
    const id_snapshot = try frame.snapshotStatePreservingMoveIds(node);

    if (node._parent) |old_parent| {
        if (frame.removeNode(old_parent, node, .{
            .will_be_reconnected = connected,
            .preserve_browsing_contexts = true,
        }) == .reentered) {
            try frame.migrateStatePreservingMoveIds(&id_snapshot);
            return;
        }
    }

    const insertion_result = if (ref) |r|
        frame.insertNodeRelative(self, node, .{ .before = r }, .{
            .child_already_connected = connected,
            .preserve_browsing_contexts = true,
        })
    else
        frame.appendNode(self, node, .{
            .child_already_connected = connected,
            .preserve_browsing_contexts = true,
        });
    insertion_result catch |err| {
        try frame.migrateStatePreservingMoveIds(&id_snapshot);
        return err;
    };
    ...
    if (connected) {
        // Atomic-move reactions cover the shadow-including subtree.
        try frame.enqueueConnectedMoveCallbacks(node);
    }
}
...
const CloneError = error{
    ...
    JsException,
    SecurityError,
    TransparentPrototypeFunctionMissing,
    TransparentPrototypeMissing,
    TransparentPrototypeInvalid,
    TransparentPrototypeSetFailed,
    NullPrototypeSetFailed,
};
...
fn _normalize(self: *Node, allocator: Allocator, buffer: *std.ArrayList(u8), frame: *Frame) !void {
    ...
    while (child) |current_node| {
        ...
        if (text_node._proto.getData().len == 0) {
            _ = frame.removeNode(self, current_node, .{ .will_be_reconnected = false });
            child = next_node;
            continue;
        }
        ...
        if (next_node) |next| {
            if (next.is(CData.Text)) |_| {
                ...
                while (next_node) |node_to_merge| {
                    ...
                    _ = frame.removeNode(self, to_remove, .{ .will_be_reconnected = false });
                }
...
pub fn replaceChildren(self: *Node, nodes: []const NodeOrText, frame: *Frame) !void {
    ...
    // Snapshot the original set. iframe discard can complete the containing
    // document's load and run script; that script may move a sibling which a
    // linked-list iterator had already cached. Revalidate each snapshot entry
    // before removal and leave callback-created children to the nested mutation.
    ...
    for (original_children.items) |child| {
        if (child._parent != self) continue;
        _ = frame.removeNode(self, child, .{ .will_be_reconnected = false });
    }
...
pub const NodeOrText = union(enum) {
    node: *Node,
    // Explicit DOMString prevents TypedArray values from being selected as a
    // BufferSource merely because both adapters can use byte slices in Zig.
    text: js.DOMString,
    ...
    pub fn format(self: *const NodeOrText, writer: *std.io.Writer) !void {
        switch (self.*) {
            ...
            .text => |text| {
                ...
                try writer.writeAll(text.value);
                try writer.writeByte('\'');
            },
        }
    }
    ...
    pub fn toNode(self: *const NodeOrText, frame: *Frame) !*Node {
        return switch (self.*) {
            .node => |n| n,
            .text => |txt| Frame.node_factory.createTextNode(frame, txt.value),
        };
    }
