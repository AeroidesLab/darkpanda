...
pub fn createObjectURL(blob: *Blob, exec: *const Execution) ![]const u8 {
    ...
    const serialized_origin = objectURLOrigin(exec.requestOrigin());
    switch (exec.js.global) {
        inline else => |g| {
            const blob_url = try std.fmt.allocPrint(
                g.arena,
                "blob:{s}/{s}",
                .{ serialized_origin, uuid_buf },
            );
            try exec.page.registerBlobURL(&g._blob_urls, g.arena, blob_url, blob);
            return blob_url;
        },
    }
}

fn createObjectURLBinding(blob_value: ?js.Value, exec: *Execution) ![]const u8 {
    const operation: js.WebIDL.Operation = .{
        .interface = "URL",
        .name = "createObjectURL",
    };
    const value = blob_value orelse
        return js.WebIDL.requiredArgument(exec, operation, 1, 0);
    const blob = value.toZig(*Blob) catch
        return js.WebIDL.typeError(exec, operation, "Overload resolution failed.");
    return createObjectURL(blob, exec);
}

fn objectURLOrigin(origin: HttpClient.RequestOrigin) []const u8 {
...
pub const JsApi = struct {
    ...
    pub const createObjectURL = bridge.function(URL.createObjectURLBinding, .{ .static = true, .arity = 1 });
    pub const revokeObjectURL = bridge.function(URL.revokeObjectURL, .{ .static = true });
    // URL's generated prototype preserves this Chrome 149 IDL order.
...
test "WebApi: object URL serializes opaque creator as blob null" {
    try std.testing.expectEqualStrings("https://example.test", objectURLOrigin(.{
        .tuple = "https://example.test",
    }));
    try std.testing.expectEqualStrings("null", objectURLOrigin(.@"opaque"));
}
