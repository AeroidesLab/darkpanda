...
pub fn getOrigin(self: *const WorkerLocation, exec: *const js.Execution) ![]const u8 {
    // A blob URL's origin is the origin of its inner URL. rust-url provides
    // this for the URL interface, but this lightweight WorkerLocation path
    // uses the browser URL helper directly and must unwrap it explicitly.
    if (std.ascii.startsWithIgnoreCase(self._url, "blob:")) {
        return (try U.getOrigin(exec.call_arena, self._url["blob:".len.. :0])) orelse "null";
    }
