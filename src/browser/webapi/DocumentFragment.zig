...
pub const JsApi = struct {
    ...
    // DocumentFragment is a visible link in ShadowRoot's prototype chain.
    // Keep its string-key order aligned with Chrome 149; innerHTML belongs to
    // ShadowRoot, not DocumentFragment.
    ...
    pub const append = bridge.function(DocumentFragment.append, .{ .ce_reactions = true, .variadic = true });
    ...
    pub const prepend = bridge.function(DocumentFragment.prepend, .{ .ce_reactions = true, .variadic = true });
    ...
    pub const replaceChildren = bridge.function(DocumentFragment.replaceChildren, .{ .ce_reactions = true, .variadic = true });
};
