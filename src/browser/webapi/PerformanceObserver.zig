...
fn initBinding(callback: js.Value, exec: *Execution) !*PerformanceObserver {
    ...
    const function = js.Function{
        ...
        .handle = @ptrCast(callback.handle),
    };
    return init(try function.persist(), exec);
}
        ...
        if (options.type) |entry_type| {
            ...
            if (options.entryTypes != null) {
                return js.WebIDL.typeError(
                    @constCast(exec),
                    .{ .interface = "PerformanceObserver", .name = "observe" },
                    "An observe() call must not include both entryTypes and type arguments.",
                );
            }
...
pub fn takeRecords(self: *PerformanceObserver) ![]*Performance.Entry {
    ...
    self._entries.clearRetainingCapacity();
    Performance.sortEntries(records);
    return records;
}

pub fn getSupportedEntryTypes(exec: *const Execution) []const []const u8 {
    // Blink derives this FrozenArray from the realm's Performance instance.
    // A DedicatedWorker has no Window-only producers, so Chrome exposes only
    // the three worker-capable entry classes there.
    return switch (exec.js.global) {
...
pub const JsApi = struct {
    ...
    pub const constructor = bridge.constructor(PerformanceObserver.initBinding, .{});
    ...
    pub const supportedEntryTypes = bridge.accessor(PerformanceObserver.getSupportedEntryTypes, null, .{
        .static = true,
        .cache = .{ .private = "performance_observer_supported_entry_types" },
    });
};
