...
/// `rel` and iframe `sandbox` define supported-token sets; per spec every
/// other backing attribute throws. Loaders probe relList while capability
/// code commonly probes iframe.sandbox.supports().
...
pub const JsApi = struct {
    ...
    pub const add = bridge.function(DOMTokenList.add, .{ .ce_reactions = true, .variadic = true });
    pub const remove = bridge.function(DOMTokenList.remove, .{ .ce_reactions = true, .variadic = true });
