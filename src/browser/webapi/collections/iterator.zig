...
pub fn Entry(comptime Inner: type, comptime field: ?[]const u8) type {
    ...
    return struct {
        _inner: Inner,
        // Iterators are standalone Factory allocations.  A Window execution
        // uses Page.factory, while a DedicatedWorker execution uses its
        // thread-owned Factory.  Page is still required by an inner
        // collection's releaseRef(), but it does not identify which slab owns
        // this iterator.
        _factory: *Factory,
        _rc: lp.RC(u8) = .{},
        ...
        pub fn init(inner: Inner, executor: R.Executor) !*Self {
            const self = try executor._factory.create(Self{
                ._inner = inner,
                ._factory = executor._factory,
            });
