...
/// Error a stream with the original JavaScript reason. Transform algorithms
/// are specified to reject the writer and readable side with the same object;
/// reducing it to a Zig error name or string is observably incorrect.
pub fn doErrorValue(self: *ReadableStreamDefaultController, err: js.Value) !void {
    if (self._stream._state != .readable) return;

    self._stream._state = .errored;
    self._stream._stored_error_value = try err.persist();
    self._queue.clearRetainingCapacity();

    const local = self._execution.js.local.?;
    for (self._pending_reads.items) |resolver| {
        local.toLocal(resolver).reject("stream error value", err);
    }
    self._pending_reads.clearRetainingCapacity();
}
