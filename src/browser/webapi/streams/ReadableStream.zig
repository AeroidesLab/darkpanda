...
_stored_error_value: ?js.Value.Global,
// Fetch/Body calls this the disturbed bit. It differs from `locked`: merely
// acquiring a reader locks a stream, while the first read/cancel disturbs it.
_disturbed: bool = false,
...
pub fn pipeTo(self: *ReadableStream, destination: *WritableStream, exec: *const Execution) !js.Promise {
    if (self.getLocked()) {
        return js.WebIDL.rejectedTypeError(
            @constCast(exec),
            .{ .interface = "ReadableStream", .name = "pipeTo" },
...
fn pipeToBinding(self: *ReadableStream, destination_value: ?js.Value, exec: *Execution) !js.Promise {
    const operation: js.WebIDL.Operation = .{
        .interface = "ReadableStream",
        .name = "pipeTo",
    };
    const value = destination_value orelse return js.WebIDL.rejectedTypeError(
        exec,
        operation,
        "1 argument required, but only 0 present.",
    );
    const destination = value.toZig(*WritableStream) catch
        return js.WebIDL.rejectedTypeError(
            exec,
            operation,
            "parameter 1 is not of type 'WritableStream'.",
        );
    return pipeTo(self, destination, exec);
}
...
const PipeState = struct {
    ...
    fn onReadFulfilled(self: *PipeState, data_: ?ReadData) void {
        ...
        if (data.done) {
            ...
            self.writable.closeStream(exec) catch |err| {
                return self.fail(local, &try_catch, err);
            };
        ...
        try_catch.init(local);
        defer try_catch.deinit();
        self.writable.writeChunk(value, exec) catch |err| {
            return self.fail(local, &try_catch, err);
        };
    ...
    fn fail(self: *PipeState, local: *const js.Local, try_catch: *js.TryCatch, err: anyerror) void {
        const reason = self.writable.failOperation(local, try_catch, err) catch {
            return self.finish(local);
        };
        self.reader.releaseLock();
        if (self.resolver) |r| {
            local.toLocal(r).reject("pipe transform failed", reason);
        }
    }
...
pub const JsApi = struct {
    ...
    pub const Meta = struct {
        ...
        pub const prototype_aliases = [_]js.bridge.PrototypeAlias{
            .{
                .source = "values",
                .target = .{ .well_known_symbol = .async_iterator },
            },
        };
    ...
    pub const pipeTo = bridge.function(ReadableStream.pipeToBinding, .{
        .arity = 1,
        .receiver_mode = .reject_promise,
    });
    pub const values = bridge.function(ReadableStream.getAsyncIterator, .{});
};
