...
pub const ZigContextTransformFn = *const fn (*anyopaque, *TransformStreamDefaultController, js.Value) anyerror!void;
pub const ZigContextFlushFn = *const fn (*anyopaque, *TransformStreamDefaultController) anyerror!void;

/// Native transformers need per-instance state just like their JavaScript
/// counterparts. Keeping the state opaque here lets Web APIs implement their
/// algorithms without teaching the generic streams layer about encodings (or
/// any other concrete transform).
pub const ZigTransformer = struct {
    context: *anyopaque,
    transform: ZigContextTransformFn,
    flush: ?ZigContextFlushFn = null,
};
...
pub fn initWithZigTransformer(zig_transformer: ZigTransformer, exec: *const Execution) !*TransformStream {
    ...
    const transform_controller = try TransformStreamDefaultController.init(self, null, null, null, exec);
    transform_controller._zig_transformer = zig_transformer;
    self._controller = transform_controller;
...
pub fn transformWrite(self: *TransformStream, chunk: js.Value, exec: *const Execution) !void {
    if (self._controller._zig_transformer) |transformer| {
        try transformer.transform(transformer.context, self._controller, chunk);
        return;
    }
    ...
    if (self._controller._transform_fn) |transform_fn| {
        ...
        try ls.toLocal(transform_fn).callRethrow(void, .{ chunk, self._controller });
    } else {
...
pub fn transformClose(self: *TransformStream, exec: *const Execution) !void {
    if (self._controller._zig_transformer) |transformer| {
        if (transformer.flush) |flush| {
            try flush(transformer.context, self._controller);
        }
    }
    ...
    if (self._controller._flush_fn) |flush_fn| {
        ...
        try ls.toLocal(flush_fn).callRethrow(void, .{self._controller});
    }
...
pub fn errorWithValue(self: *TransformStream, reason: js.Value) !void {
    try self._writable.errorWithValue(reason);
    try self._readable._controller.doErrorValue(reason);
}
