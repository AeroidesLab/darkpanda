...
pub const InitError = error{InvalidRequestURL};
...
pub fn init(input: Input, opts_: ?InitOpts, exec: *const Execution) !*Request {
    return initInternal(input, opts_, exec) catch |err| switch (err) {
        error.InvalidRequestURL => {
            const reason = try invalidURLReason(input, exec.call_arena);
            return js.WebIDL.constructorTypeError(@constCast(exec), "Request", reason);
        },
        else => return err,
    };
}

/// Fetch must turn Request construction failures into a rejected promise,
/// whereas `new Request()` throws synchronously. Keep the parse sentinel until
/// Fetch has installed its operation-qualified TypeError.
pub fn initForFetch(input: Input, opts_: ?InitOpts, exec: *const Execution) !*Request {
    return initInternal(input, opts_, exec);
}

pub fn invalidURLReason(input: Input, allocator: Allocator) ![]const u8 {
    const source = switch (input) {
        .url => |url| url,
        .request => unreachable,
    };
    return std.fmt.allocPrint(allocator, "Failed to parse URL from {s}", .{source});
}

fn initInternal(input: Input, opts_: ?InitOpts, exec: *const Execution) !*Request {
    const arena = exec.arena;
    const url = switch (input) {
        .url => |u| URL.resolve(arena, exec.base(), u, .{ .encoding = exec.charset.* }) catch |err| switch (err) {
            error.TypeError => return error.InvalidRequestURL,
            else => return err,
        },
    ...
    var headers = if (opts.headers) |headers_init|
        try Headers.init(headers_init, exec)
    else switch (input) {
        .url => null,
        .request => |r| if (r._headers) |source_headers|
            try Headers.init(.{ .obj = source_headers }, exec)
        else
            null,
    };
    ...
    const body = if (opts.body) |b| blk: {
        ...
        if (extracted.content_type) |ct| {
            ...
            if (!(try hs.has("content-type", exec))) {
...
pub fn clone(self: *const Request, exec: *const Execution) !*Request {
    const headers = if (self._headers) |source_headers|
        try Headers.init(.{ .obj = source_headers }, exec)
    else
        null;
