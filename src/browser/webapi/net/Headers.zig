    ...
    const list = blk: {
        ...
        switch (opts) {
            ...
            .js_obj => |js_obj| {
                ...
                try validateConstructorHeaders(&list, exec);
                break :blk list;
            },
            .strings => |kvs| {
...
pub fn append(self: *Headers, name: []const u8, value: []const u8, exec: *const Execution) !void {
    ...
    const normalized_name = try validateAndNormalizeOperationName(name, "append", exec);
    try validateOperationValue(normalized_value, "append", exec);
    try self.ensureMutable("append", exec);
...
pub fn delete(self: *Headers, name: []const u8, exec: *const Execution) !void {
    const normalized_name = try validateAndNormalizeOperationName(name, "delete", exec);
    try self.ensureMutable("delete", exec);
    self._list.delete(normalized_name, null);
}

pub fn get(self: *const Headers, name: []const u8, exec: *const Execution) !?[]const u8 {
    const normalized_name = try validateAndNormalizeOperationName(name, "get", exec);
    return self.getByNormalizedName(normalized_name, exec.call_arena);
}

/// Native callers with a known-valid, already-lowercase header name can avoid
/// expanding their error set with the script-facing Web IDL validation error.
pub fn getByNormalizedName(
    self: *const Headers,
    normalized_name: []const u8,
    allocator: Allocator,
) Allocator.Error!?[]const u8 {
    const all_values = try self._list.getAll(allocator, normalized_name);
    ...
    return try std.mem.join(allocator, ", ", all_values);
}

pub fn has(self: *const Headers, name: []const u8, exec: *const Execution) !bool {
    const normalized_name = try validateAndNormalizeOperationName(name, "has", exec);
    return self._list.has(normalized_name);
}

pub fn set(self: *Headers, name: []const u8, value: []const u8, exec: *const Execution) !void {
    const normalized_value = normalizeHeaderValue(value);
    const normalized_name = try validateAndNormalizeOperationName(name, "set", exec);
    try validateOperationValue(normalized_value, "set", exec);
    try self.ensureMutable("set", exec);
    try self._list.set(exec.arena, normalized_name, normalized_value);
}

pub fn setGuard(self: *Headers, guard: Guard) void {
    self._guard = guard;
}

fn ensureMutable(self: *const Headers, operation: []const u8, exec: *const Execution) !void {
    if (self._guard != .immutable) return;
    return js.WebIDL.typeError(
        @constCast(exec),
        .{ .interface = "Headers", .name = operation },
...
pub fn populateHttpHeader(self: *Headers, allocator: Allocator, http_headers: *http.Headers) !void {
    for (self._list._entries.items) |entry| {
        ...
        // A script-visible Headers object may retain these names, but Fetch
        // and XHR must not let author code replace browser/network metadata.
        // Chrome silently ignores forbidden request headers at send time.
        if (isForbiddenRequestHeaderName(name)) continue;
        const merged = try std.mem.concatWithSentinel(allocator, u8, &.{ entry.name.str(), ": ", entry.value.str() }, 0);
        // Fetch/XHR author headers replace generated defaults (notably
        // Accept) rather than creating a duplicate. Headers.set preserves the
        // caller's spelling and places the override at its final wire-order
        // position.
        try http_headers.set(merged);
    }
}

fn isForbiddenRequestHeaderName(name: []const u8) bool {
    if (std.ascii.startsWithIgnoreCase(name, "proxy-") or
        std.ascii.startsWithIgnoreCase(name, "sec-")) return true;
...
fn validateAndNormalizeOperationName(
    name: []const u8,
    operation: []const u8,
    exec: *const Execution,
) ![]const u8 {
    validateHeaderName(name) catch return js.WebIDL.typeError(
        @constCast(exec),
        .{ .interface = "Headers", .name = operation },
        "Invalid name",
    );
...
fn validateHeaderName(name: []const u8) error{TypeError}!void {
    if (!Mime.isHttpToken(name)) return error.TypeError;
}

fn normalizeHeaderValue(value: []const u8) []const u8 {
    return std.mem.trim(u8, value, &Mime.HTTP_WHITESPACE);
}

fn validateOperationValue(
    value: []const u8,
    operation: []const u8,
    exec: *const Execution,
) !void {
    validateHeaderValue(value) catch return js.WebIDL.typeError(
        @constCast(exec),
        .{ .interface = "Headers", .name = operation },
        "Invalid value",
    );
...
fn validateConstructorHeaders(list: *KeyValueList, exec: *const Execution) !void {
    validateAndNormalizeConstructor(list) catch |err| switch (err) {
        error.InvalidHeaderName => return js.WebIDL.constructorTypeError(
            @constCast(exec),
            "Headers",
            "Invalid name",
        ),
        error.InvalidHeaderValue => return js.WebIDL.constructorTypeError(
            @constCast(exec),
            "Headers",
            "Invalid value",
        ),
...
const ConstructorValidationError = error{
    InvalidHeaderName,
    InvalidHeaderValue,
};
...
fn validateAndNormalizeConstructor(list: *KeyValueList) ConstructorValidationError!void {
    for (list._entries.items) |*entry| {
        // A valid header name is exactly a non-empty HTTP token.
        validateHeaderName(entry.name.str()) catch return error.InvalidHeaderName;
        const trimmed = entry.value.trim(&Mime.HTTP_WHITESPACE);
        validateHeaderValue(trimmed.str()) catch return error.InvalidHeaderValue;
        entry.value = trimmed;
    }
}
