...
pub fn send(self: *XMLHttpRequest, body_: ?BodyInit, exec_: *const Execution) !void {
    ...
    if (self._exec.isShuttingDown()) {
        const message = try std.fmt.allocPrint(
            exec_.call_arena,
            "Failed to execute 'send' on 'XMLHttpRequest': Failed to load '{s}': Document is already detached.",
            .{self._url},
        );
        const local = exec_.js.local orelse return error.NetworkError;
        const exception = try local.zigValueToJs(DOMException.init(message, "NetworkError"), .{});
    ...
    if (body_) |b| {
        if (self._method != .GET and self._method != .HEAD) {
            ...
            if (extracted.content_type) |ct| {
                if (!(try self._request_headers.has("content-type", exec_))) {
    ...
    var headers = try http_client.newRequestHeaders(self._url, .{
        .destination = .empty,
        .mode = .cors,
        .initiator_url = exec.url.*,
        .top_level_url = exec.url.*,
        .referrer_url = exec.url.*,
        .method = self._method,
        .has_body = self._request_body != null,
        .credentials = if (self._with_credentials) .include else .same_origin,
    });
