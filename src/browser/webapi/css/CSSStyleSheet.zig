...
// Last text-node content observed for an owner <style>. CSSOM mutations can
// intentionally diverge from that text; only a changed signature reparses it.
_owner_text_signature: ?OwnerTextSignature = null,

const OwnerTextSignature = struct {
    hash: u64,
    len: usize,

    fn init(text: []const u8) OwnerTextSignature {
        return .{ .hash = std.hash.Wyhash.hash(0, text), .len = text.len };
    }
};
...
pub fn getCssRules(self: *CSSStyleSheet, frame: *Frame) !*CSSRuleList {
    ...
    if (self.getOwnerNode()) |owner| {
        if (owner.is(Element.Html.Style)) |style| {
            ...
            self._owner_text_signature = .init(text);
...
/// Synchronize an owner <style>'s CSSOM rule list only when its text children
/// changed. Merely touching unrelated DOM must preserve insertRule/deleteRule
/// changes that intentionally do not rewrite the element's textContent.
pub fn refreshOwnerTextIfChanged(self: *CSSStyleSheet, frame: *Frame) !void {
    const owner = self.getOwnerNode() orelse return;
    const style = owner.is(Element.Html.Style) orelse return;
    const text = try style.asNode().getTextContentAlloc(frame.local_arena);
    const signature: OwnerTextSignature = .init(text);
    if (self._owner_text_signature) |previous| {
        if (previous.hash == signature.hash and previous.len == signature.len) return;
    }
    self._owner_text_signature = signature;

    // If cssRules has never been observed, StyleManager can parse owner text
    // directly without materializing the full CSSOM graph.
    if (self._css_rules == null) return;
    try self.replaceSync(text, frame);
}
...
pub fn insertRule(self: *CSSStyleSheet, rule: []const u8, maybe_index: ?u32, frame: *Frame) !u32 {
    ...
    const inserted: *CSSRule = switch (parsed_rule) {
        .style => |s| blk: {
            ...
            try style.setCssTextFromParser(s.block, frame);
