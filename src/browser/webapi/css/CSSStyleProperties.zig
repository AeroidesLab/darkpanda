...
pub fn setNamed(self: *CSSStyleProperties, name: []const u8, value: []const u8, frame: *Frame) !void {
    if (method_names.has(name) or !CSSPropertyRegistry.hasAccessorName(name)) {
        return error.NotHandled;
    }
    const dash_case = camelCaseToDashCase(name, &frame.buf);
    try self._proto.denyComputedNamedMutation(name, dash_case, frame);
...
pub fn getNamed(self: *CSSStyleProperties, name: []const u8, frame: *Frame) ![]const u8 {
    // Named CSSOM access uses exact Web IDL supported-property spellings. Do
    // this bounded registry check before camelCaseToDashCase writes Frame.buf;
    // property keys are otherwise arbitrary-length author input.
    if (method_names.has(name) or !CSSPropertyRegistry.hasAccessorName(name)) {
        return error.NotHandled;
    }
    ...
    // Property accessors have special handling for empty values: supported
    // properties (including Blink's supported -webkit- surface) return '',
    // while unknown and unsupported vendor-prefixed names return undefined.
    if (value.len == 0) {
...
fn isKnownCSSProperty(dash_case: []const u8) bool {
    ...
    return CSSPropertyRegistry.hasCssName(dash_case) or known_properties.has(dash_case);
}

pub fn getIndex(self: *CSSStyleProperties, index: u32) ?[]const u8 {
    if (index >= self._proto.length()) return null;
    return self._proto.item(index);
}

pub fn hasIndex(self: *CSSStyleProperties, index: u32) ?u32 {
    if (index >= self._proto.length()) return null;
    return @intCast(js.v8.ReadOnly);
}

pub fn getIndexes(self: *CSSStyleProperties, exec: *const js.Execution) !js.Array {
    const len = self._proto.length();
...
pub fn hasNamed(_: *CSSStyleProperties, name: []const u8) bool {
    if (method_names.has(name)) return false;
    return CSSPropertyRegistry.hasAccessorName(name);
}

pub fn getNamedKeys(_: *CSSStyleProperties, exec: *const js.Execution) !js.Array {
    var keys = exec.js.local.?.newArray(CSSPropertyRegistry.named_keys.len);
    for (CSSPropertyRegistry.named_keys, 0..) |name, index| {
...
pub const JsApi = struct {
    ...
    pub const Meta = struct {
        ...
        // CSSStyleProperties is a Zig implementation detail used to host the
        // CSSOM named/indexed interceptors. Blink exposes the wrapped object as
        // a CSSStyleDeclaration directly, without an intermediate prototype.
        pub const transparent_prototype = true;
        pub const prototype_chain = bridge.prototypeChain();
        pub var class_id: bridge.ClassId = undefined;
    };

    pub const @"[int]" = bridge.indexedReadWrite(CSSStyleProperties.getIndex, null, null, CSSStyleProperties.hasIndex, CSSStyleProperties.getIndexes, .{ .null_as_undefined = true });
    pub const @"[str]" = bridge.namedIndexed(CSSStyleProperties.getNamed, CSSStyleProperties.setNamed, null, CSSStyleProperties.getNamedKeys, CSSStyleProperties.hasNamed, .{});
};
