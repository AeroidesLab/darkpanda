...
pub const Type = union(enum) {
    ...
    performance: *@import("Performance.zig"),
    ...
    rtc_data_channel: *@import("RTCPeerConnection.zig").RTCDataChannel,
    cookie_store: *@import("storage/CookieStore.zig"),
    idb_request: *@import("storage/idb/IDBRequest.zig"),
    idb_open_db_request: *@import("storage/idb/IDBOpenDBRequest.zig"),
    ...
    offscreen_canvas: *@import("canvas/OffscreenCanvas.zig"),
    network_information: *@import("NavigatorCapabilities.zig").NetworkInformation,
...
pub fn format(self: *EventTarget, writer: *std.Io.Writer) !void {
    return switch (self._type) {
        ...
        .performance => writer.writeAll("<Performance>"),
        ...
        .offscreen_canvas => writer.writeAll("<OffscreenCanvas>"),
...
pub fn toString(self: *EventTarget) []const u8 {
    return switch (self._type) {
        ...
        .offscreen_canvas => return "[object OffscreenCanvas]",
...
pub const JsApi = struct {
    ...
    // The callback is nullable in Web IDL, but it is still a required second
    // argument. Keep Function.length aligned with Chrome/EventTarget.idl.
    pub const addEventListener = bridge.function(EventTarget.addEventListener, .{ .arity = 2, .required_args = 2 });
    pub const removeEventListener = bridge.function(EventTarget.removeEventListener, .{ .arity = 2, .required_args = 2 });
};
