...
fn getHeaders(
    ...
    return self.base.getHeaders(target_url, mode);
}
...
pub fn preloadScript(self: *ScriptManager, element: *Element.Html, url: []const u8) !bool {
    ...
    try frame.makeRequest(.{
        ...
        .headers = try self.base.getHeaders(owned_url, .no_cors),
...
pub fn addFromElement(self: *ScriptManager, comptime from_parser: bool, script_element: *Element.Html.Script, comptime ctx: []const u8) !void {
    ...
    const cors_enabled = kind == .module or
        element.getAttributeSafe(comptime .wrap("crossorigin")) != null;
    const request_mode: HttpClient.HeaderRequestContext.Mode = if (cors_enabled) .cors else .no_cors;
    ...
    script.* = .{
        ...
        .muted_errors = if (remote_url) |url|
            kind == .javascript and !cors_enabled and !frame.isSameOrigin(url)
        else
            false,
        .extra = .{ .frame = .{
            .kind = kind,
        ...
        if (is_blocking) {
            if (self.waitForPreload(url)) |pre| {
                ...
                const response = try self.base.client.syncRequest(arena, .{
                    ...
                    .frame_id = frame._frame_id,
                    .loader_id = frame._loader_id,
                    .headers = try self.getHeaders(url, request_mode),
                    .cookie_jar = &frame._session.cookie_jar,
                    .cookie_origin = frame.url,
                    .resource_type = .script,
                    .notification = frame._session.notification,
                    .resource_timing = frame.performance().resourceTimingSink(&frame.js.execution, .script),
                });
