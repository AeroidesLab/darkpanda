...
const ErrorEvent = @import("webapi/event/ErrorEvent.zig");
...
/// Reserve one position in the target's event-listener registration timeline.
/// HTML event-handler properties use this alongside addEventListener entries:
/// replacing a non-null handler keeps its old position, while clearing and
/// assigning it again creates a new position at the end.
pub fn reserveRegistrationOrder(self: *EventManagerBase) u64 {
    const order = self.next_registration_order;
...
pub fn register(self: *EventManagerBase, target: *EventTarget, typ: []const u8, callback: Callback, opts: RegisterOptions) !?*Listener {
    ...
    listener.* = .{
        ...
        .signal = opts.signal,
        .typ = type_string,
        .registration_order = self.reserveRegistrationOrder(),
    };
...
pub const DispatchDirectOptions = struct {
    ...
    // Most DOM dispatch entry points are standalone script boundaries and run
    // a checkpoint when their listeners finish. Lifecycle dismissal events
    // are dispatched as one cluster, so their caller can suppress the
    // per-event checkpoint and let the surrounding task drain once.
    microtask_checkpoint: bool = true,
};

pub const OrderedHandlerKind = enum {
    event,
    window_error,
};

/// A property event handler which participates in the same registration
/// timeline as addEventListener callbacks. The slots are read again at the
/// handler's turn so replacement-without-clearing invokes the replacement;
/// clearing/reassigning changes the order and therefore skips this dispatch.
pub const OrderedHandler = struct {
    function_slot: *const ?js.Function.Global,
    registration_order_slot: *const ?u64,
    kind: OrderedHandlerKind = .event,
};
...
pub fn dispatchDirect(
    ...
    // A direct target still runs the DOM dispatch algorithm's AT_TARGET
    // phase. Keep a one-entry path alive while author callbacks run so
    // eventPhase/currentTarget/composedPath match Window, IDB, XHR and other
    // non-Node EventTargets in Chromium. Register this cleanup after the
    // Local.Scope defer: LIFO ordering clears the dispatch state before the
    // listener microtask checkpoint.
    const path = try arena.alloc(Event.DispatchPathEntry, 1);
    path[0] = .{
        .invocation_target = target,
        .adjusted_target = event._target orelse target,
        .adjusted_related_target = event.getRelatedTarget(),
        .closed_roots = &.{},
    };
    ...
    const ordered_handler_snapshot: ?u64 = if (comptime @TypeOf(handler) == OrderedHandler)
        handler.registration_order_slot.*
    else
        null;
    var ordered_handler_pending = ordered_handler_snapshot != null;

    // Legacy direct-dispatch handlers have always occupied the first slot.
    // Ordered handlers instead merge with the listener list below.
    if (comptime @TypeOf(handler) != OrderedHandler) {
        if (getFunction(handler, &ls.local)) |func| {
            invokeDirectEventHandler(func, target, event, opts.context);
            if (event._stop_immediate_propagation) return;
        }
    }

    // No listeners were registered via addEventListener at dispatch start.
    const last_node = sentinel orelse {
        if (comptime @TypeOf(handler) == OrderedHandler) {
            if (ordered_handler_pending) {
                invokeOrderedHandler(handler, ordered_handler_snapshot.?, &ls.local, target, event, opts.context);
            }
        }
        return;
    };
    ...
    while (node) |n| {
        ...
        if (comptime @TypeOf(handler) == OrderedHandler) {
            if (ordered_handler_pending and ordered_handler_snapshot.? < listener.registration_order) {
                invokeOrderedHandler(handler, ordered_handler_snapshot.?, &ls.local, target, event, opts.context);
                ordered_handler_pending = false;
                if (event._stop_immediate_propagation) return;
            }
        }
...
fn invokeOrderedHandler(
    ...
    // A null/reassign during an earlier listener is a newly registered handler
    // and must not enter the listener snapshot for this dispatch.
    if ((handler.registration_order_slot.* orelse return) != registration_order) return;
    const global = handler.function_slot.* orelse return;
    const func = local.toLocal(global);

    switch (handler.kind) {
        .event => invokeDirectEventHandler(func, target, event, context),
        .window_error => {
            const error_event = event.as(ErrorEvent);
            event._current_target = target;
            // HTML's special Window error handler is invoked with the realm's
            // actual global object as `this`. Mapping the inherited native
            // EventTarget pointer would expose the base wrapper instead of the
            // WindowProxy/global identity.
            const result = func.callWithThis(js.Value, local.getGlobal(), .{
                error_event._message,
                error_event._filename,
                error_event._line_number,
                error_event._column_number,
                local.toLocal(error_event._error),
            }) catch |err| {
                if (err == error.JsException) {
                    event._listeners_did_throw = true;
                } else {
            ...
            // The legacy Window.onerror cancellation convention is exact
            // primitive true, not truthiness.
            if (result.isTrue()) event.preventDefault();
        },
    }
}
...
fn hasHandler(handler: anytype) bool {
    if (comptime @TypeOf(handler) == OrderedHandler) {
        return handler.registration_order_slot.* != null and handler.function_slot.* != null;
    }
...
pub const Listener = struct {
    ...
    // Per DOM section 2.9 step 4 substep 8 ("Inner invoke"), a listener callback that
