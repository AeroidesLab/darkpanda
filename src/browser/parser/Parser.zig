...
fn beginHTMLDocumentParse(self: *Parser) void {
    const document = self.container.node.is(Node.Document) orelse return;
    if (self.iframe_srcdoc) {
        document.lockNoQuirksCompatibilityMode();
    } else {
        document.beginHTMLDocumentParse();
    }
}

pub fn parse(self: *Parser, html: []const u8) void {
    self.beginHTMLDocumentParse();
...
pub const Streaming = struct {
    ...
    pub fn start(self: *Streaming) !void {
        ...
        self.parser.beginHTMLDocumentParse();
...
fn setQuirksModeCallback(ctx: *anyopaque, mode: u8) callconv(.c) void {
    const self: *Parser = @ptrCast(@alignCast(ctx));
    const document = self.container.node.is(Node.Document) orelse return;
    document.setHTMLCompatibilityMode(mode);
}
fn _appendDoctypeToDocument(self: *Parser, name: []const u8, public_id: []const u8, system_id: []const u8) !void {
    ...
    // html5ever's set_quirks_mode callback is the sole source of the
    // document compatibility state. This callback only creates the node.
...
fn _appendCallback(self: *Parser, parent: *Node, node_or_text: h5e.NodeOrText) !void {
    ...
    switch (node_or_text.toUnion()) {
        .node => |cpn| {
            ...
            const parent_connected = parent.isConnected();
            var load_guard = self.frame.beginReconnectLoadGuard(child._parent != null and child_connected and parent_connected);
            ...
            if (child._parent) |previous_parent| {
                ...
                if (self.frame.removeNode(previous_parent, child, .{ .will_be_reconnected = parent_connected }) == .reentered) return;
            }
...
fn _appendBeforeSiblingCallback(self: *Parser, sibling: *Node, node_or_text: h5e.NodeOrText) !void {
    ...
    const child_connected = node.isConnected();
    const parent_connected = parent.isConnected();
    var load_guard = self.frame.beginReconnectLoadGuard(node._parent != null and child_connected and parent_connected);
    defer load_guard.deinit();
    if (node._parent) |previous_parent| {
        // A custom element constructor may have inserted the node into the DOM
        // before the parser officially places it (e.g. via foster parenting).
        // Detach it first so insertNodeRelative's assertion holds.
        if (self.frame.removeNode(previous_parent, node, .{ .will_be_reconnected = parent_connected }) == .reentered) return;
    }
