...
/// Chromium's schemeful-site comparison uses the registrable domain (including
/// private suffixes). The generated PSL table already backs SameSite cookies;
/// expose the same calculation for navigation permissions.
pub fn registrableDomain(host: []const u8) []const u8 {
    return domainAndRegistry(host) orelse host;
}
...
fn prefixedPublicSuffixRule(prefix: []const u8, value: []const u8) bool {
    // A DNS hostname is at most 253 bytes. Leave a little room for the PSL's
    // wildcard/exception marker without allocating in a navigation check.
    var rule_buf: [256]u8 = undefined;
    if (prefix.len + value.len > rule_buf.len) return false;
    @memcpy(rule_buf[0..prefix.len], prefix);
    @memcpy(rule_buf[prefix.len .. prefix.len + value.len], value);
    return public_suffix_list(rule_buf[0 .. prefix.len + value.len]);
}

fn publicSuffixLabelCount(host: []const u8) usize {
    // PSL's implicit fallback rule is "*", so an unknown final label is still
    // a one-label registry. Exact/private rules, wildcard rules and exception
    // rules then compete using the longest matching rule.
    var best: usize = 1;
    var start: usize = 0;
    while (start < host.len) {
        const candidate = host[start..];
        const candidate_labels = labelCount(candidate);

        // An exception rule wins and removes its left-most label from the
        // public suffix. For !www.ck, ck is the suffix and www.ck is a site.
        if (prefixedPublicSuffixRule("!", candidate))
            return candidate_labels - 1;

        if (public_suffix_list(candidate))
            best = @max(best, candidate_labels);

        if (std.mem.indexOfScalar(u8, candidate, '.')) |dot| {
            const wildcard_base = candidate[dot + 1 ..];
            if (prefixedPublicSuffixRule("*.", wildcard_base))
                best = @max(best, candidate_labels);
            start += dot + 1;
        } else break;
    }
...
/// Blink's network_utils::GetDomainAndRegistry(...,
/// kIncludePrivateRegistries). Unlike the cookie-oriented convenience helper
/// above, this returns null for IP literals, single-label hosts, and hosts that
/// are themselves a public suffix. Unknown final labels are treated as a
/// registry, matching Chromium's INCLUDE_UNKNOWN_REGISTRIES behavior.
pub fn domainAndRegistry(host: []const u8) ?[]const u8 {
    if (host.len == 0 or isIpLiteral(host)) return null;
    const host_labels = labelCount(host);
    if (host_labels < 2) return null;

    const public_suffix_labels = publicSuffixLabelCount(host);
    if (host_labels <= public_suffix_labels) return null;

    var labels_to_skip = host_labels - public_suffix_labels - 1;
    var start: usize = 0;
    while (labels_to_skip > 0) : (labels_to_skip -= 1) {
        const dot = std.mem.indexOfScalarPos(u8, host, start, '.') orelse return null;
        start = dot + 1;
    }
    return host[start..];
}

test "URL: Chrome domain and registry excludes non-site hosts" {
    try std.testing.expect(domainAndRegistry("127.0.0.1") == null);
    try std.testing.expect(domainAndRegistry("[::1]") == null);
    try std.testing.expect(domainAndRegistry("localhost") == null);
    try std.testing.expect(domainAndRegistry("com") == null);
    try std.testing.expect(domainAndRegistry("co.uk") == null);
    try std.testing.expectEqualStrings("example.com", domainAndRegistry("example.com").?);
    try std.testing.expectEqualStrings("example.com", domainAndRegistry("sub.example.com").?);
    try std.testing.expectEqualStrings("foo.blogspot.com", domainAndRegistry("foo.blogspot.com").?);
    try std.testing.expectEqualStrings("foo.internal", domainAndRegistry("foo.internal").?);
    try std.testing.expect(domainAndRegistry("foo.ck") == null);
    try std.testing.expectEqualStrings("bar.foo.ck", domainAndRegistry("bar.foo.ck").?);
    try std.testing.expectEqualStrings("www.ck", domainAndRegistry("www.ck").?);
    try std.testing.expectEqualStrings("www.ck", domainAndRegistry("sub.www.ck").?);
}
...
// Returns true if these two *canonical serialized* URLs point to the same
// document.  This mirrors Blink's EqualIgnoringFragmentIdentifier(KURL, KURL):
// KURL parsing/canonicalization happens before this byte comparison.
...
/// Compare canonical fragment identifiers for Location.hash's early-return
/// rule. Blink converts both fragment StringViews to strings and uses
/// EqualIgnoringNullity, so an absent fragment and an empty fragment (`#`) are
/// equal here even though their full URL serializations differ.
pub fn eqlFragmentIdentifierIgnoringNullity(first: [:0]const u8, second: [:0]const u8) bool {
    const first_start = if (std.mem.indexOfScalar(u8, first, '#')) |index| index + 1 else first.len;
    const second_start = if (std.mem.indexOfScalar(u8, second, '#')) |index| index + 1 else second.len;
    return std.mem.eql(u8, first[first_start..], second[second_start..]);
}
...
test "URL: canonical document and Location fragment comparisons" {
    defer testing.reset();

    // Special-URL backslashes are normalized by KURL/rust-url before Blink's
    // EqualIgnoringFragmentIdentifier comparison.
    const windows_path = try resolve(
        testing.arena_allocator,
        "http://127.0.0.1:9582/",
        "src\\browser\\tests\\window\\location.html#one",
        .{},
    );
    try testing.expectString(
        "http://127.0.0.1:9582/src/browser/tests/window/location.html#one",
        windows_path,
    );
    try testing.expect(eqlDocument(
        windows_path,
        "http://127.0.0.1:9582/src/browser/tests/window/location.html#two",
    ));

    try testing.expect(eqlFragmentIdentifierIgnoringNullity(
        "https://example.test/path",
        "https://example.test/path#",
    ));
    try testing.expect(eqlFragmentIdentifierIgnoringNullity(
        "https://example.test/path#same",
        "https://example.test/other#same",
    ));
    try testing.expect(!eqlFragmentIdentifierIgnoringNullity(
        "https://example.test/path#old",
        "https://example.test/path#new",
    ));
    try testing.expect(eqlFragmentIdentifierIgnoringNullity(
        "https://example.test/path#a%20b",
        "https://example.test/path#a%20b",
    ));
}
