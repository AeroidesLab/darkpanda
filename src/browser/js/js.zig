...
pub const CSPTrustedTypes = @import("CSPTrustedTypes.zig");
pub const Execution = @import("Execution.zig");
pub const Local = @import("Local.zig");
pub const Inspector = @import("Inspector.zig");
pub const Snapshot = @import("Snapshot.zig");
pub const Platform = @import("Platform.zig");
pub const ForegroundWake = @import("ForegroundWake.zig");
...
pub const WebIDL = @import("WebIDL.zig");
...
pub fn newTrackedSlot(ctx: *Context, handle: anytype) !*GlobalSlot {
    ...
    return ctx.env.globalTracker(ctx.page).track(global);
}
...
// Explicit marker for DOMString arms inside Web IDL unions. A bare
// `[]const u8` is also used for BufferSource adapters, so union probing cannot
// safely infer string semantics from that Zig type alone.
pub const DOMString = struct {
    value: []const u8,
};

// DOMString adapter for the Web IDL [LegacyNullToEmptyString] extended
// attribute. It still uses the invocation's conversion context for every
// non-null value, including Symbol failures.
pub const LegacyNullToEmptyDOMString = struct {
    value: []const u8,
};
...
test "ForegroundWake: released and empty wrappers fail closed" {
    var wake: ForegroundWake = .{ .inner = .{ .handle = null } };
    try std.testing.expect(!wake.post());
    try std.testing.expect(!wake.close());
    wake.deinit();
    try std.testing.expect(wake.inner.handle == null);
}
...
pub const FinalizerCallback = struct {
    ...
    pub const Identity = struct {
        ...
        // Stable handles to the registry and pool this struct came from. A
        // DedicatedWorker uses Env-owned instances; Window uses Page/Browser
        // instances. Both outlive their isolate's final weak callback.
        registry: *Registry,
        pool: *std.heap.MemoryPool(@This()),
    ...
    pub const Registry = std.AutoHashMapUnmanaged(usize, *FinalizerCallback);
