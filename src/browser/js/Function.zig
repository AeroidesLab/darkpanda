...
pub fn callRethrowWithThis(self: *const Function, comptime T: type, this: anytype, args: anytype) !T {
    var caught: js.TryCatch.Caught = undefined;
    return self._tryCallWithThis(T, this, args, &caught, .{ .rethrow = true }) catch |err| {
        log.warn(.js, "callRethrowWithThis caught", .{ .err = err, .caught = caught });
        return err;
    };
}
