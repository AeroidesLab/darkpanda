...
pub fn stackLimit(self: Isolate) usize {
    return v8.v8__Isolate__GetStackLimit(self.handle);
}

pub fn swapStackLimit(self: Isolate, stack_limit: usize) usize {
    return v8.v8__Isolate__SwapStackLimit(self.handle, stack_limit);
}

pub fn stackLimitFromCurrentPosition(self: Isolate, budget: usize) usize {
    _ = self;
    return v8.v8__Stack__GetCurrentStackPosition() - budget;
}
...
pub fn memoryPressureNotification(self: Isolate, level: MemoryPressureLevel) void {
    // Clang imports a C enum as signed int on MSVC but unsigned int on some
    // ELF targets. Let the call's parameter type drive the cast so this ABI
    // wrapper remains source-identical across Windows, Linux and macOS.
...
pub fn createWasmCompileError(self: Isolate, msg: []const u8) *const v8.Value {
    const message = self.initStringHandle(msg);
    return v8.v8__Exception__WasmCompileError(message).?;
}
