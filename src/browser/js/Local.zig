...
pub fn mapZigInstanceToJs(self: *const Local, js_obj_handle: ?*const v8.Object, value: anytype) !js.Object {
    ...
    switch (@typeInfo(T)) {
        .@"struct" => {
            ...
            // A wrapper can outlive the native callback which created it.
            // Most Web API value structs contain only owned/context-lifetime
            // data, but a few (notably DOMException) can be initialized from
            // call-arena strings.  Let those types deep-copy transient fields
            // before the wrapper stores their native pointer.
            heap.* = if (comptime @hasDecl(T, "cloneForJs"))
                try value.cloneForJs(context_arena)
            else
                value;
        ...
        .pointer => |ptr| {
            ...
            // Some Zig implementation types need their own InstanceTemplate
            // for internal fields or named/indexed interceptors, but Chromium
            // exposes the object with the exact public parent prototype. Keep
            // FunctionTemplate::Inherit so V8 receiver Signatures recognize the
            // child map, then change only the newly-created object's visible
            // prototype. V8's prototype transition preserves the child map's
            // constructor template, descriptors, interceptors and layout.
            // `value` is frequently typed as a base union such as *Element;
            // resolveValue() then selects the concrete implementation from its
            // `_type` tag.  Therefore this decision must use the resolved class
            // id, not the compile-time type of the incoming pointer.  The latter
            // works for CSSStyleProperties (returned directly) but misses an
            // SVGGenericElement returned through *Element.
            if (transparentPrototypeParentIndexForClassId(resolved.class_id)) |parent_index| {
                const parent_template = ctx.templates[parent_index];
                const parent_function = v8.v8__FunctionTemplate__GetFunction(
                    parent_template,
                    self.handle,
                ) orelse return error.TransparentPrototypeFunctionMissing;
                const prototype_key = self.isolate.initStringHandle("prototype");
                const parent_prototype = v8.v8__Object__Get(
                    @ptrCast(parent_function),
                    self.handle,
                    prototype_key,
                ) orelse return error.TransparentPrototypeMissing;
                if (!v8.v8__Value__IsObject(parent_prototype)) {
                    return error.TransparentPrototypeInvalid;
                }

                var prototype_set: v8.MaybeBool = undefined;
                v8.v8__Object__SetPrototype(
                    js_obj.handle,
                    self.handle,
                    @ptrCast(parent_prototype),
                    &prototype_set,
                );
                if (!prototype_set.value) return error.TransparentPrototypeSetFailed;
            }

            // Cross-origin WindowProxy/Location views are platform exotics,
            // not instances of a public Web IDL interface. Chromium exposes a
            // null visible prototype for both while V8 keeps the native
            // FunctionTemplate and internal fields behind the scenes. The C
            // binding's Object parameter is ABI-compatible with the Value
            // accepted by SetPrototypeV2, so the JavaScript null oddball is a
            // valid prototype value here.
            if (nullPrototypeForClassId(resolved.class_id)) {
                var prototype_set: v8.MaybeBool = undefined;
                v8.v8__Object__SetPrototype(
                    js_obj.handle,
                    self.handle,
                    @ptrCast(self.isolate.initNull()),
                    &prototype_set,
                );
                if (!prototype_set.value) return error.NullPrototypeSetFailed;
            }
            ...
            // A small number of Blink objects perform per-wrapper work after
            // the native pointer has been associated. Dispatch by the resolved
            // concrete class because callers can arrive through a base pointer
            // whose compile-time JsApi is not the wrapper being instantiated.
            try associateWithWrapperForClassId(self, resolved.class_id, resolved.ptr, js_obj);
            ...
            if (resolved.finalizer) |finalizer| {
                ...
                const page = ctx.page;
                const resources = ctx.env.finalizerResources(page);
                const finalizer_gop = try resources.registry.getOrPut(resources.allocator, finalizer_ptr_id);
                if (finalizer_gop.found_existing == false) {
                    ...
                    errdefer _ = resources.registry.remove(finalizer_ptr_id);
                ...
                const identity_finalizer = try resources.identity_pool.create();
                identity_finalizer.* = .{
                    .registry = resources.registry,
                    .pool = resources.identity_pool,
                    .page = page,
...
pub fn jsValueToZig(self: *const Local, comptime T: type, js_val: js.Value) !T {
    return self.jsValueToZigWithContext(T, js_val, null);
}
...
pub fn jsValueToZigWithContext(
    ...
    comptime T: type,
    js_val: js.Value,
    context: ?js.WebIDL.ConversionContext,
) !T {
    switch (@typeInfo(T)) {
        .optional => |o| {
        ...
        .pointer => |ptr| switch (ptr.size) {
            ...
            .slice => {
                ...
                for (arr, 0..) |*a, i| {
                    ...
                    a.* = try self.jsValueToZigWithContext(ptr.child, item_value, context);
                }
                return arr;
            },
            else => {},
        ...
        .array => |arr| {
            ...
            const slice_value = try self.jsValueToZigWithContext(slice_type, js_val, context);
        ...
        .@"struct" => {
            return try (self.jsValueToStruct(T, js_val, context)) orelse {
                return error.InvalidArgument;
            };
        },
        .@"union" => |u| {
            ...
            inline for (u.fields, 0..) |field, i| {
                switch (try self.probeJsValueToZig(field.type, js_val)) {
                    ...
                    .ok => {
                        ...
                        return @unionInit(T, field.name, try self.jsValueToZigWithContext(field.type, js_val, context));
                    },
...
fn jsValueToStruct(
    ...
    return switch (T) {
        js.DOMString => .{ .value = try js.WebIDL.toDOMStringWithContext(js_val, &self.ctx.execution, context) },
        js.LegacyNullToEmptyDOMString => .{
            .value = if (js_val.isNull())
                ""
            else
                try js.WebIDL.toDOMStringWithContext(js_val, &self.ctx.execution, context),
        },
        ...
        string.Global => try js_val.toSSO(true),
            ...
            inline for (@typeInfo(T).@"struct".fields) |field| {
                ...
                if (js_obj.has(key)) {
                    @field(value, name) = try self.jsValueToZigWithContext(field.type, try js_obj.get(key), context);
...
fn probeJsValueToZig(self: *const Local, comptime T: type, js_val: js.Value) !ProbeResult(T) {
    switch (@typeInfo(T)) {
        ...
        .@"struct" => {
            if (T == js.DOMString or T == js.LegacyNullToEmptyDOMString) {
                if (v8.v8__Value__IsString(js_val.handle)) {
                    return .{ .ok = {} };
                }
                // Web IDL DOMString uses ToString. TypedArray/Array values are
                // therefore coercible strings here, never BufferSource views.
                return .{ .coerce = {} };
            }
            ...
            const value = (try self.jsValueToStruct(T, js_val, null)) orelse {
                return .{ .invalid = {} };
            };
            return .{ .value = value };
        },
...
fn usesTransparentPrototype(comptime JsApi: type) bool {
    return @hasDecl(JsApi.Meta, "transparent_prototype") and
        JsApi.Meta.transparent_prototype;
}

fn transparentPrototypeParentIndex(comptime JsApi: type) ?bridge.JsApiLookup.BackingInt {
    const T = JsApi.bridge.type;
    if (!@hasField(T, "_proto")) return null;
    const ParentPtr = std.meta.fieldInfo(T, ._proto).type;
    const Parent = @typeInfo(ParentPtr).pointer.child;
    return bridge.JsApiLookup.getId(Parent.JsApi);
}

fn transparentPrototypeParentIndexForClassId(class_id: u16) ?bridge.JsApiLookup.BackingInt {
    inline for (bridge.JsApis, 0..) |JsApi, i| {
        if (comptime usesTransparentPrototype(JsApi)) {
            const parent_index = comptime transparentPrototypeParentIndex(JsApi) orelse
                @compileError(@typeName(JsApi) ++ " uses Meta.transparent_prototype without a _proto parent");
            if (class_id == i) return parent_index;
        }
    }
    return null;
}

fn nullPrototypeForClassId(class_id: u16) bool {
    inline for (bridge.JsApis, 0..) |JsApi, i| {
        if (comptime @hasDecl(JsApi.Meta, "null_prototype") and JsApi.Meta.null_prototype) {
            if (class_id == i) return true;
        }
    }
    return false;
}

fn associateWithWrapperForClassId(
    local: *const Local,
    class_id: u16,
    ptr: *anyopaque,
    wrapper: js.Object,
) error{TypeError}!void {
    inline for (bridge.JsApis, 0..) |JsApi, i| {
        if (class_id == i) {
            if (comptime @hasDecl(JsApi, "associateWithWrapper")) {
                const T = JsApi.bridge.type;
                const instance: *T = @ptrCast(@alignCast(ptr));
                try JsApi.associateWithWrapper(instance, wrapper, local);
            }
            return;
        }
    }
    // class_id is produced from the same JsApis table.
    unreachable;
}
        ...
        .finalizer = blk: {
            ...
            const Wrap = struct {
                ...
                fn releaseRef(handle: ?*const v8.WeakCallbackInfo) callconv(.c) void {
                    ...
                    defer identity_finalizer.pool.destroy(identity_finalizer);
                    ...
                    const fc = identity_finalizer.registry.get(finalizer_ptr_id) orelse return;
                    ...
                    if (fc.identity_count == 0) {
                        ...
                        _ = identity_finalizer.registry.remove(finalizer_ptr_id);
...
pub const Scope = struct {
    ...
    pub fn deinit(self: *Scope) void {
        ...
        _ = self.local.isolate.swapStackLimit(self.previous_stack_limit);
