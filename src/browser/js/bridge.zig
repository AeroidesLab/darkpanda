...
/// Selects which objects created from an interface template have an immutable
/// [[Prototype]].  Keeping this independent from access checks is important:
/// [LegacyUnforgeable] and security-boundary semantics are separate Web IDL
/// concepts even though Location happens to use both.
pub const ImmutableProto = enum {
    instance,
    prototype,
    both,
};

/// A realm-local intrinsic which V8 resolves while instantiating a template.
/// These must not be captured as concrete values while creating the startup
/// snapshot: doing that would leak the snapshot realm's intrinsic identity
/// into every subsequently-created Window or Worker realm.
pub const TemplateIntrinsic = enum {
    array_prototype_entries,
    array_prototype_for_each,
...
pub const WellKnownSymbol = enum {
    async_iterator,
    has_instance,
...
/// Describes a data property installed directly on every interface instance.
/// `phase` makes ordering explicit because Reflect.ownKeys observes whether a
/// property is installed before or after the interface's IDL members.
pub const InstanceTemplateProperty = struct {
    key: Key,
    value: Value,
    writable: bool = true,
    enumerable: bool = true,
    configurable: bool = true,
    phase: Phase = .after_members,
...
/// Reuses one realized native prototype member under another key.  Web IDL
/// iterable declarations require strict function identity (for example
/// ReadableStream.prototype.values === @@asyncIterator); creating a second
/// FunctionTemplate would only reproduce behavior, not identity.
pub const PrototypeAlias = struct {
    source: []const u8,
    target: InstanceTemplateProperty.Key,
    writable: bool = true,
...
pub fn Builder(comptime T: type) type {
    return struct {
        ...
        /// Install one [LegacyUnforgeable] attribute directly on every
        /// interface instance. Unlike Meta.legacy_unforgeable_members this is
        /// per-member, as required by attributes such as Document.location.
        pub fn legacyUnforgeableAccessor(comptime getter: anytype, comptime setter: anytype, comptime opts: Caller.Function.Opts) Accessor {
            if (opts.static) @compileError("a LegacyUnforgeable accessor cannot be static");
            var result = Accessor.init(T, getter, setter, opts);
            result.legacy_unforgeable = true;
            return result;
        }

        /// A real Web IDL accessor whose getter returns a profile-independent
        /// literal.  This is intentionally distinct from `property`, which is
        /// reserved for IDL constants/data properties.
        pub fn constantAccessor(comptime value: anytype) Accessor {
            const getter = switch (@typeInfo(@TypeOf(value))) {
                .null => struct {
                    fn get(_: *const T) ?u8 {
                        return null;
                    }
                ...
                .pointer => |ptr| blk: {
                    if (ptr.size != .slice and !(ptr.size == .one and @typeInfo(ptr.child) == .array and @typeInfo(ptr.child).array.child == u8)) {
                        @compileError("constantAccessor only accepts string pointers");
                    }
                    break :blk struct {
                        fn get(_: *const T) @TypeOf(value) {
                            return value;
                ...
                else => @compileError("constantAccessor for " ++ @typeName(@TypeOf(value)) ++ " has not been defined"),
            };
            return Accessor.init(T, getter, null, .{});
        }
        ...
        pub fn indexed(comptime getter_func: anytype, comptime enumerator_func: anytype, comptime opts: Indexed.Opts) Indexed {
            return Indexed.init(T, getter_func, null, null, null, enumerator_func, null, null, opts);
        }
        ...
        pub fn indexedReadWrite(comptime getter_func: anytype, setter_func: anytype, deleter_func: anytype, query_func: anytype, comptime enumerator_func: anytype, comptime opts: Indexed.Opts) Indexed {
            return Indexed.init(T, getter_func, setter_func, deleter_func, query_func, enumerator_func, null, null, opts);
        }

        /// Indexed getter with the Web IDL legacy-platform-object behavior for
        /// an interface that declares no indexed setter/deleter.
        pub fn indexedLegacyReadOnly(
            comptime getter_func: anytype,
            query_func: anytype,
            comptime enumerator_func: anytype,
            comptime interface_name: []const u8,
            comptime opts: Indexed.Opts,
        ) Indexed {
            var legacy_opts = opts;
            legacy_opts.legacy_readonly_interface = interface_name;
            return Indexed.init(T, getter_func, null, null, query_func, enumerator_func, null, null, legacy_opts);
        }

        // Full indexed-property exotic hook set. Definer/descriptor callbacks
        // are needed when defineProperty/getOwnPropertyDescriptor participate
        // in the object's observable access-control surface.
        pub fn indexedFull(
            comptime getter_func: anytype,
            setter_func: anytype,
            deleter_func: anytype,
            query_func: anytype,
            comptime enumerator_func: anytype,
            definer_func: anytype,
            descriptor_func: anytype,
            comptime opts: Indexed.Opts,
        ) Indexed {
            return Indexed.init(
                T,
        ...
        /// Named getter with the Web IDL legacy-platform-object behavior for
        /// an interface that declares no named setter/deleter.
        pub fn namedIndexedLegacyReadOnly(
            comptime getter_func: anytype,
            enumerator_func: anytype,
            query_func: anytype,
            comptime interface_name: []const u8,
            comptime opts: NamedIndexed.Opts,
        ) NamedIndexed {
            var legacy_opts = opts;
            legacy_opts.legacy_readonly_interface = interface_name;
            return NamedIndexed.init(T, getter_func, null, null, enumerator_func, query_func, null, null, legacy_opts);
        }

        // Full named-property exotic hook set.  Most Web IDL named getters do
        // not need definer/descriptor interception; security-boundary objects
        // do, because Object.defineProperty/getOwnPropertyDescriptor are part
        // of their observable access-control surface.
        pub fn namedIndexedFull(
            comptime getter_func: anytype,
            setter_func: anytype,
            deleter_func: anytype,
            enumerator_func: anytype,
...
pub const Constructor = struct {
    ...
    const Opts = struct {
        ...
        // Explicit Web IDL metadata for adapters whose Zig optionality differs
        // from their author-visible constructor signature.
        arity: ?usize = null,
        required_args: ?usize = null,
        // Explicit because constructors may also use slices for ordinary
        // sequence/BufferSource parameters.
        variadic: bool = false,
    };

    fn init(comptime T: type, comptime func: anytype, comptime opts: Opts) Constructor {
        const required_args = comptime (opts.required_args orelse Function.getRequiredArity(
            @TypeOf(func),
            if (opts.new_target) 1 else 0,
            opts.variadic,
        ));
        return .{
            .arity = @intCast(comptime (opts.arity orelse if (opts.variadic)
                required_args
            else
                Function.getArity(@TypeOf(func), if (opts.new_target) 1 else 0))),
            .func = struct {
...
pub const Function = struct {
    ...
    receiver_mode: Caller.Function.Opts.ReceiverMode = .v8_signature,
    func: *const fn (?*const v8.FunctionCallbackInfo) callconv(.c) void,
    ...
    fn init(comptime T: type, comptime func: anytype, comptime opts: Caller.Function.Opts) Function {
        if (comptime opts.receiver_mode == .reject_promise) {
            if (opts.static or opts.embedded_receiver) {
                @compileError("reject_promise receiver mode requires an ordinary instance operation");
            }
            if (opts.cache != null) {
                @compileError("reject_promise receiver mode cannot be combined with a bridge cache");
            }
            const declared_return = @typeInfo(@TypeOf(func)).@"fn".return_type orelse
                @compileError("reject_promise receiver mode requires a return type");
            const return_type = switch (@typeInfo(declared_return)) {
                .error_union => |eu| eu.payload,
                else => declared_return,
            };
            if (return_type != js.Promise) {
                @compileError("reject_promise receiver mode is only valid for operations returning js.Promise");
            }
        }
        // Always instantiate the inference path so `.variadic = true` gets
        // compile-time shape validation even when required_args is overridden.
        const inferred_required_args = comptime getRequiredArity(
            @TypeOf(func),
            if (opts.static) 0 else 1,
            opts.variadic,
        );
        const required_args = comptime (opts.required_args orelse inferred_required_args);
        return .{
            .cache = opts.cache,
            .receiver_mode = opts.receiver_mode,
            .static = opts.static,
            ...
            .arity = opts.arity orelse if (opts.variadic)
                required_args
            else
                getArity(@TypeOf(func), if (opts.static) 0 else 1),
            .func = if (opts.noop) noopFunction else struct {
                fn wrap(handle: ?*const v8.FunctionCallbackInfo) callconv(.c) void {
                    Caller.Function.call(T, handle.?, func, withRequiredArgs(opts, required_args));
                }
            }.wrap,
        };
    }

    fn withRequiredArgs(comptime opts: Caller.Function.Opts, comptime required_args: usize) Caller.Function.Opts {
        var result = opts;
        result.required_args = required_args;
        return result;
    }
    ...
    fn getRequiredArity(comptime T: type, comptime start: usize, comptime variadic: bool) usize {
        const Execution = js.Execution;
        ...
        var end = params.len;
        if (end > start) {
            const Last = params[end - 1].type.?;
            if (Last == *Frame or Last == *const Frame or
                Last == *Page or Last == *const Page or
                Last == *Execution or Last == *const Execution or
                Last == *Session or Last == *const Session)
            {
                end -= 1;
            }
        }
        ...
        if (variadic) {
            if (end <= start) {
                @compileError("a variadic Web IDL operation must expose a trailing JavaScript parameter");
            }
            const Variadic = params[end - 1].type.?;
            const info = @typeInfo(Variadic);
            if (info != .pointer or info.pointer.size != .slice) {
                @compileError("a variadic Web IDL operation must use a trailing Zig slice adapter");
            }
            end -= 1;
        }

        var count: usize = 0;
        for (params[start..end]) |param| {
            const PT = param.type.?;
            if (@typeInfo(PT) == .optional) break;
            count += 1;
        }
        return count;
    }
...
pub const Accessor = struct {
    ...
    fn init(comptime T: type, comptime getter: anytype, comptime setter: anytype, comptime opts: Caller.Function.Opts) Accessor {
        ...
        if (@typeInfo(@TypeOf(getter)) != .null) {
            const getter_opts = blk: {
                var o = opts;
                o.ce_reactions = false;
                o.webidl_callback_kind = .attribute_get;
                o.cross_origin_allowed = opts.cross_origin_getter_allowed orelse opts.cross_origin_allowed;
                break :blk o;
            };
        ...
        if (@typeInfo(@TypeOf(setter)) != .null) {
            const setter_opts = blk: {
                var o = opts;
                o.webidl_callback_kind = .attribute_set;
                o.cross_origin_allowed = opts.cross_origin_setter_allowed orelse opts.cross_origin_allowed;
                break :blk o;
            };
            accessor.setter = struct {
                fn wrap(handle: ?*const v8.FunctionCallbackInfo) callconv(.c) void {
                    Caller.Function.call(T, handle.?, setter, setter_opts);
                }
            }.wrap;
        }
...
pub const Indexed = struct {
    ...
    enumerator: ?*const fn (handle: ?*const v8.PropertyCallbackInfo) callconv(.c) void,
    ...
    definer: ?*const fn (idx: u32, descriptor: ?*const v8.PropertyDescriptor, handle: ?*const v8.PropertyCallbackInfo) callconv(.c) u32 = null,
    descriptor: ?*const fn (idx: u32, handle: ?*const v8.PropertyCallbackInfo) callconv(.c) u32 = null,

    const Opts = struct {
        as_typed_array: bool = false,
        null_as_undefined: bool = false,
        legacy_readonly_interface: ?[]const u8 = null,
    };
    ...
    fn init(
        ...
        if (@typeInfo(@TypeOf(enumerator)) != .null) {
            indexed.enumerator = struct {
                fn wrap(handle: ?*const v8.PropertyCallbackInfo) callconv(.c) void {
                    const v8_isolate = v8.v8__PropertyCallbackInfo__GetIsolate(handle).?;
                    var caller: Caller = undefined;
                    if (!caller.init(v8_isolate)) {
                        return;
                    ...
                    defer caller.deinit();
                    _ = caller.getEnumerator(T, enumerator, handle.?, .{});
                }
            }.wrap;
        }
        ...
        if (@typeInfo(@TypeOf(definer)) != .null) {
            indexed.definer = struct {
                fn wrap(idx: u32, _: ?*const v8.PropertyDescriptor, handle: ?*const v8.PropertyCallbackInfo) callconv(.c) u32 {
                    const v8_isolate = v8.v8__PropertyCallbackInfo__GetIsolate(handle).?;
                    var caller: Caller = undefined;
                    if (!caller.init(v8_isolate)) {
                        return js.Intercepted.no;
                    ...
                    // Security-boundary definers reject the operation as a
                    // whole and intentionally ignore the proposed descriptor.
                    return caller.deleteIndex(T, definer, idx, handle.?, .{});
                }
            }.wrap;
        }

        if (@typeInfo(@TypeOf(descriptor)) != .null) {
            indexed.descriptor = struct {
                fn wrap(idx: u32, handle: ?*const v8.PropertyCallbackInfo) callconv(.c) u32 {
                    ...
                    return caller.getIndex(T, descriptor, idx, handle.?, .{});
                }
            }.wrap;
        }

        if (comptime opts.legacy_readonly_interface != null) {
            if (@typeInfo(@TypeOf(setter)) != .null or
                @typeInfo(@TypeOf(deleter)) != .null or
                @typeInfo(@TypeOf(definer)) != .null or
                @typeInfo(@TypeOf(query)) == .null)
            {
                @compileError("legacy readonly indexed properties require a query and no setter/deleter/definer");
            }
            const interface_name = opts.legacy_readonly_interface.?;
            indexed.setter = struct {
                fn wrap(idx: u32, _: ?*const v8.Value, handle: ?*const v8.PropertyCallbackInfo) callconv(.c) u32 {
                    const v8_isolate = v8.v8__PropertyCallbackInfo__GetIsolate(handle).?;
                    var caller: Caller = undefined;
                    if (!caller.init(v8_isolate)) return js.Intercepted.no;
                    defer caller.deinit();
                    return caller.legacyReadOnlyIndexedSetter(idx, handle.?, interface_name);
                }
            }.wrap;
            indexed.deleter = struct {
                fn wrap(idx: u32, handle: ?*const v8.PropertyCallbackInfo) callconv(.c) u32 {
                    ...
                    if (!caller.init(v8_isolate)) return js.Intercepted.no;
                    defer caller.deinit();
                    return caller.legacyReadOnlyIndexedDeleter(T, query, idx, handle.?, interface_name);
                }
            }.wrap;
            indexed.definer = struct {
                fn wrap(idx: u32, descriptor_handle: ?*const v8.PropertyDescriptor, handle: ?*const v8.PropertyCallbackInfo) callconv(.c) u32 {
                    const v8_isolate = v8.v8__PropertyCallbackInfo__GetIsolate(handle).?;
                    var caller: Caller = undefined;
                    if (!caller.init(v8_isolate)) return js.Intercepted.no;
                    defer caller.deinit();
                    return caller.legacyReadOnlyIndexedDefiner(idx, descriptor_handle.?, handle.?, interface_name);
                }
            }.wrap;
        }
...
pub const NamedIndexed = struct {
    ...
    enumerator: ?*const fn (handle: ?*const v8.PropertyCallbackInfo) callconv(.c) void = null,
    query: ?*const fn (c_name: ?*const v8.Name, handle: ?*const v8.PropertyCallbackInfo) callconv(.c) u32 = null,
    definer: ?*const fn (c_name: ?*const v8.Name, descriptor: ?*const v8.PropertyDescriptor, handle: ?*const v8.PropertyCallbackInfo) callconv(.c) u32 = null,
    ...
    fn init(
        ...
        const definer_fn = if (@typeInfo(@TypeOf(definer)) == .null) null else struct {
            fn wrap(c_name: ?*const v8.Name, _: ?*const v8.PropertyDescriptor, handle: ?*const v8.PropertyCallbackInfo) callconv(.c) u32 {
                const v8_isolate = v8.v8__PropertyCallbackInfo__GetIsolate(handle).?;
                var caller: Caller = undefined;
                if (!caller.init(v8_isolate)) {
                    return js.Intercepted.no;
                ...
                // Security-boundary definers reject the operation as a whole;
                // they intentionally do not inspect the proposed descriptor.
        ...
        const legacy_setter_fn = if (opts.legacy_readonly_interface == null) setter_fn else struct {
            fn wrap(c_name: ?*const v8.Name, _: ?*const v8.Value, handle: ?*const v8.PropertyCallbackInfo) callconv(.c) u32 {
                const v8_isolate = v8.v8__PropertyCallbackInfo__GetIsolate(handle).?;
                var caller: Caller = undefined;
                if (!caller.init(v8_isolate)) return js.Intercepted.no;
                defer caller.deinit();
                return caller.legacyReadOnlyNamedSetter(T, query, c_name.?, handle.?, opts.legacy_readonly_interface.?);
            }
        }.wrap;
        const legacy_deleter_fn = if (opts.legacy_readonly_interface == null) deleter_fn else struct {
            fn wrap(c_name: ?*const v8.Name, handle: ?*const v8.PropertyCallbackInfo) callconv(.c) u32 {
                const v8_isolate = v8.v8__PropertyCallbackInfo__GetIsolate(handle).?;
                var caller: Caller = undefined;
                if (!caller.init(v8_isolate)) return js.Intercepted.no;
                defer caller.deinit();
                return caller.legacyReadOnlyNamedDeleter(T, query, c_name.?, handle.?, opts.legacy_readonly_interface.?);
            }
        }.wrap;
        const legacy_definer_fn = if (opts.legacy_readonly_interface == null) definer_fn else struct {
            fn wrap(c_name: ?*const v8.Name, _: ?*const v8.PropertyDescriptor, handle: ?*const v8.PropertyCallbackInfo) callconv(.c) u32 {
                const v8_isolate = v8.v8__PropertyCallbackInfo__GetIsolate(handle).?;
                var caller: Caller = undefined;
                if (!caller.init(v8_isolate)) return js.Intercepted.no;
                defer caller.deinit();
                return caller.legacyReadOnlyNamedDefiner(T, query, c_name.?, handle.?, opts.legacy_readonly_interface.?);
            }
        }.wrap;

        if (comptime opts.legacy_readonly_interface != null and
            (@typeInfo(@TypeOf(setter)) != .null or
                @typeInfo(@TypeOf(deleter)) != .null or
                @typeInfo(@TypeOf(definer)) != .null or
                @typeInfo(@TypeOf(query)) == .null))
        {
            @compileError("legacy readonly named properties require a query and no setter/deleter/definer");
        }

        return .{
            .getter = getter_fn,
            .setter = legacy_setter_fn,
            .deleter = legacy_deleter_fn,
            .enumerator = enumerator_fn,
            .query = query_fn,
            .definer = legacy_definer_fn,
            .descriptor = descriptor_fn,
        };
    }
};

// Cross-origin WindowProxy surfaces need to return the exact accessor
// functions from their named descriptor callback without invoking JavaScript
// recursively from inside V8.  Snapshot stores the same FunctionTemplate
// under these unreachable hidden-prototype keys; the runtime descriptor
// callback materializes the stable per-context Function from there.
pub fn crossOriginGetterBackingName(comptime name: []const u8) [:0]const u8 {
    return "\x01darkpanda:get:" ++ name;
}

pub fn crossOriginSetterBackingName(comptime name: []const u8) [:0]const u8 {
    return "\x01darkpanda:set:" ++ name;
}
...
pub fn unknownWindowPropertyCallback(c_name: ?*const v8.Name, handle: ?*const v8.PropertyCallbackInfo) callconv(.c) u32 {
    ...
    switch (local.ctx.global) {
        .frame => |frame| {
            // Window named access first resolves document-tree child browsing
            // contexts by their browsing-context name. This is deliberately
            // before document named access, so a frame beats an element with
            // the same exposed name. Shadow-scoped children are filtered by
            // Window.getNamedFrame.
            if (frame.window.getNamedFrame(property)) |child_window| {
                const js_val = local.zigValueToJs(child_window, .{}) catch return js.Intercepted.no;
...
pub const PageJsApis = flattenTypes(&.{
    ...
    @import("../webapi/InputDeviceCapabilities.zig"),
    ...
    @import("../webapi/RTCPeerConnection.zig").RTCDataChannel,
    @import("../webapi/RTCPeerConnection.zig").RTCIceCandidate,
    @import("../webapi/RTCPeerConnection.zig").RTCSessionDescription,
    @import("../webapi/RTCPeerConnection.zig").RTCPeerConnectionIceEvent,
    @import("../webapi/RTCPeerConnection.zig").RTCDataChannelEvent,
    ...
    // Navigation Timing is [Exposed=Window]; keep it out of Performance's
    // shared registerTypes() so Worker snapshots do not gain the constructor.
    @import("../webapi/Performance.zig").NavigationTiming,
    @import("../webapi/Performance.zig").TimingConfidence,
    ...
    @import("../webapi/ViewTransition.zig"),
    @import("../webapi/speech/SpeechSynthesis.zig"),
    @import("../webapi/speech/SpeechSynthesisVoice.zig"),
