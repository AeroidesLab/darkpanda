...
pub fn waitForScript(self: *Runner, frame_id: u32, src: [:0]const u8, timeout_ms: u32) !void {
    ...
    while (true) {
        ...
        // Never keep a V8 Context entered while ticking the event loop below.
        // Navigation commits can select a different WindowAgent queue, and V8
        // explicitly forbids Context::SetMicrotaskQueue while *any* Context is
        // entered on the isolate.
        const predicate_satisfied = blk: {
            var ls: js.Local.Scope = undefined;
            frame.js.localScope(&ls);
            defer ls.deinit();
