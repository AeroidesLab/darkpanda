...
pub fn init(self: *Session, browser: *Browser, notification: *Notification) !void {
    ...
    self.* = .{
        ...
        // Browser configuration defaults for this session.
...
fn allocatePage(
    ...
    try Page.init(page, self, frame_id, navigation_context);
    return page;
}
...
fn tearDownPage(self: *Session, page: *Page) void {
    ...
    self.retire(page);
    page.frame.navigation().onRemoveFrame();
}
...
fn installNewActivePage(
    self: *Session,
    frame_id: u32,
    navigation_context: ?*Frame.NavigationContext,
) !*Frame {
    const page = try self.allocatePage(frame_id, navigation_context);
    ...
    try frame.activateNavigationContext();
...
pub fn createPage(self: *Session) !PageHandle {
    ...
    _ = try self.installNewActivePage(frame_id, null);
...
pub fn closePage(self: *Session, frame_id: u32) void {
    ...
    self.removePageSessionStorageNamespaces(live);
}

fn removePageSessionStorageNamespaces(self: *Session, page: *Page) void {
    const allocator = self.browser.app.allocator;
    self.storage_shed.removeSessionNamespace(
        allocator,
        @intFromPtr(page.frame._navigation_context),
    );
    // Popups are independent top-level browsing contexts even though this
    // project currently stores them in their opener's Page allocation.
...
pub fn closeAllPages(self: *Session) void {
    ...
    for (self.pages.items) |page| {
        self.removePageSessionStorageNamespaces(page);
        page.frame.abortTransfers();
...
fn processPageQueuedNavigation(self: *Session, page: *Page) !void {
    ...
    for (navigations.items) |frame| {
        ...
        if (qn.is_inherited_about) {
            // Defer inherited about:blank/about:srcdoc to the second pass.
    ...
    while (i < new_navigations.items.len) {
        ...
        if (frame._queued_navigation) |qn| {
            if (qn.is_inherited_about) {
                log.warn(.frame, "recursive inherited about navigation", .{});
...
fn _processFrameNavigation(self: *Session, frame: *Frame, qn: *QueuedNavigation) !void {
    ...
    var owns_added_parent_delay = starts_delaying;
    errdefer {
        if (owns_added_parent_delay) parent._pending_loads -= 1;
    }

    const frame_id = frame._frame_id;
    const navigation_context = frame._navigation_context;
    const page = frame._page;
    const browsing_context_name = frame.window._name;
    const browsing_context_name_initialized = frame.window._name_initialized;
    // Snapshot the owner's latest pending frame policy before destroying the
    // current child Document. Attribute mutations affect this next
    // cross-document commit, never the Document that is being removed.
    const effective_frame_sandbox_flags = Frame.pendingSandboxFlagsForChild(parent, iframe);
    var child_index = std.mem.indexOfScalar(*Frame, parent.child_frames.items, frame) orelse
        return error.FrameNotInParent;

    // Detach the outer proxy and make the old realm inert, but retain its
    // Context/Frame/Window addresses until Page teardown. Only after that
    // ownership transfer may the replacement Context reattach the proxy.
    try page.retireFrameForNavigation(frame);
    // Detach/unload is a script boundary. Re-resolve the old frame just as
    // Chromium's Frame::SwapImpl re-checks that it remains attached before
    // replacing it in the FrameTree.
    child_index = std.mem.indexOfScalar(*Frame, parent.child_frames.items, frame) orelse {
        iframe._window = null;
        return error.FrameNotInParent;
    };
    errdefer {
        if (std.mem.indexOfScalar(*Frame, parent.child_frames.items, frame)) |index| {
            _ = parent.child_frames.orderedRemove(index);
            parent.child_frames_mutation_serial +%= 1;
        }
        iframe._window = null;
    }

    const new_frame = try page.frame_arena.create(Frame);
    try Frame.init(new_frame, frame_id, page, .{
        .parent = parent,
        .navigation_context = navigation_context,
        .effective_frame_sandbox_flags = effective_frame_sandbox_flags,
        // Frame scope belongs to the browsing context, not to the iframe's
        // current DOM position, and therefore survives cross-document commits.
        .scoped_for_window = frame.scoped_for_window,
    });
    errdefer {
        new_frame.deinit();
        if (std.mem.indexOfScalar(*Frame, parent.child_frames.items, new_frame)) |index| {
            _ = parent.child_frames.orderedRemove(index);
            parent.child_frames_mutation_serial +%= 1;
        }
        iframe._window = null;
    ...
    new_frame.window._name = browsing_context_name;
    new_frame.window._name_initialized = browsing_context_name_initialized;
    ...
    new_frame._delays_parent_load = was_delaying or starts_delaying;

    // Publish the fresh inner realm atomically into the active tree. The
    // iframe continues to expose the same V8 WindowProxy because both inner
    // Window aliases resolve through navigation_context's stable key.
    parent.child_frames.items[child_index] = new_frame;
    parent.child_frames_mutation_serial +%= 1;
    ...
    owns_added_parent_delay = false;
}

// A popup also keeps one stable outer WindowProxy, while each document commit
// gets a fresh native Frame/Window/Context inner realm.
fn processPopupNavigation(_: *Session, frame: *Frame, qn: *QueuedNavigation) !void {
    const old_window = frame.window;
    const saved_name = old_window._name;
    const saved_name_initialized = old_window._name_initialized;
    const saved_opener = old_window._opener;
    const frame_id = frame._frame_id;
    const navigation_context = frame._navigation_context;
    const page = frame._page;
    // Popup frame-policy sandbox flags are browsing-context state and persist
    // across its subsequent network navigations. Response CSP does not.
    const effective_frame_sandbox_flags = frame._effective_frame_sandbox_flags;

    const popup_index = std.mem.indexOfScalar(*Frame, page.popups.items, frame) orelse
        return error.PopupNotRegistered;

    try page.retireFrameForNavigation(frame);
    errdefer if (popup_index < page.popups.items.len and page.popups.items[popup_index] == frame) {
    ...
    errdefer {
        page.replaceOpenerReferences(new_frame.window, null);
        if (std.mem.indexOfScalar(*Frame, page.popups.items, new_frame)) |index| {
            _ = page.popups.swapRemove(index);
        }
        new_frame.deinit();
    }

    new_frame.window._name = saved_name;
    new_frame.window._name_initialized = saved_name_initialized;
    new_frame.window._opener = saved_opener;
    page.popups.items[popup_index] = new_frame;
    page.replaceOpenerReferences(old_window, new_frame.window);
...
fn processRootQueuedNavigation(self: *Session, page: *Page) !void {
    ...
    const is_synthetic = qn.is_inherited_about or std.mem.startsWith(u8, qn.url, "blob:");
...
fn replaceRootImmediate(self: *Session, frame_id: u32, url: [:0]const u8, opts: Frame.NavigateOpts) !void {
    var navigate_opts = opts;
    const old_page = self.livePage(frame_id);
    const navigation_context = if (old_page) |page| page.frame._navigation_context else null;
    var copied_state_arena: ?Allocator = null;
    defer if (copied_state_arena) |arena| self.releaseArena(arena);
    const browsing_context_name = if (old_page) |page| blk: {
        // The old Page is queued for destruction before the fresh Page owns a
        // copy, so bridge the name through a short-lived Session arena.
        const arena = try self.getArena(.tiny, "replaceRootImmediate.state");
        copied_state_arena = arena;
        break :blk try arena.dupe(u8, page.frame.window._name);
    } else "";
    const browsing_context_name_initialized = if (old_page) |page|
        page.frame.window._name_initialized
    else
        false;

    // Resolve and retain a blob URL while its creating Page/agent-cluster
    // registry is still alive. The replacement Page has a fresh registry, so
    // looking it up after tearDownPage deterministically loses the resource.
    // The parser consumes the immutable Blob synchronously during navigate().
    if (std.mem.startsWith(u8, url, "blob:")) {
        const page = old_page orelse return error.BlobNotFound;
        const blob = page.acquireBlobURL(url) orelse return error.BlobNotFound;
        defer blob.releaseRef(page);
        navigate_opts.synthetic_blob = blob;

        // A blob:null URL does not serialize its creator's opaque identity.
        // Direct/native root navigation may bypass scheduleNavigation's
        // snapshot, so copy the key while the old Page is still alive. Never
        // pass its *Origin into the replacement Page: Frame.navigate resolves
        // this owned key through the new Page's origin registry instead.
        if (navigate_opts.initiator_origin_key == null and
            std.mem.startsWith(u8, url, "blob:null/"))
        {
            const arena = copied_state_arena orelse unreachable;
            navigate_opts.initiator_origin_key = try arena.dupe(
                u8,
                page.frame.js.origin.key,
            );
        }
    }
    ...
    const new_frame = try self.installNewActivePage(frame_id, navigation_context);
    new_frame.window._name = try new_frame.arena.dupe(u8, browsing_context_name);
    new_frame.window._name_initialized = browsing_context_name_initialized;

    new_frame.navigate(url, navigate_opts) catch |err| {
...
pub fn initiateRootNavigation(self: *Session, frame_id: u32, url: [:0]const u8, opts: Frame.NavigateOpts) !void {
    ...
    // Synthetic navigations (inherited about: URLs and blob:) have no HTTP round-trip and
    ...
    if (std.mem.eql(u8, "about:blank", url) or
        std.mem.eql(u8, "about:srcdoc", url) or
        std.mem.startsWith(u8, url, "blob:"))
    {
        return self.replaceRootImmediate(frame_id, url, opts);
    }

    const page = try self.allocatePage(frame_id, live.frame._navigation_context);
    errdefer self.queuePageDestruction(page);
    // A pending HTTP Page is a fresh inner Window for the same top-level
    // browsing context. Copy the context name into its own frame arena while
    // the active Page is still alive; commit may destroy the source Page.
    page.frame.window._name = try page.frame.arena.dupe(u8, live.frame.window._name);
    page.frame.window._name_initialized = live.frame.window._name_initialized;
...
pub fn commitPendingPage(self: *Session, replacement: *Page) !void {
    ...
    self.notification.dispatch(.frame_remove, .{});
    old.frame.navigation().onRemoveFrame();
    ...
    replacement.frame.activateNavigationContext() catch |err| {
...
pub const PageHandle = struct {
    ...
    pub fn navigate(self: PageHandle, request_url: [:0]const u8, opts: Frame.NavigateOpts) !void {
        ...
        if (f.isInitialRootNavigation()) {
            return f.navigate(request_url, opts);
        }
        return self.session.initiateRootNavigation(self.frame_id, request_url, opts);
    }
};
